...
- **mode B が単独で持つ拡張**: `--type=practice` / `--feature=<slug>`（2a）、`--watch`（3f＝Tier B watchlist 登録）、`--feed=<rss_url>` / `--feed=notion-*:<sel>` / `--feed=claude-blog-sitemap:default`（3g/4/4c＝フィード登録・後 2 者は document-less な feed_registry append 特例）、host routing（4a）。
- **`--watch` / `--feed` は共通 surface 内部呼び出し（G/H/F migration）には伝播させない**＝default-off を維持し誤発火を防ぐ（D2 の default-off 方針の帰結。SKILL.md では D ラベルなしで規定）。
...
| host | 取得経路 | 備考 |
|------|----------|------|
| 一般 URL | `WebFetch`（要約） | raw に `note:` 明示＋ `source_url` 再検証経路を残す（WebFetch raw 逐語性ポリシー） |
...
**日付メタ抽出（Phase 5・経路と直交）**: raw 確保時、同じ経路から**元ドキュメント日付**もベストエフォート抽出し raw frontmatter に記録する（`published_at` / `last_modified` ＋ per-date 出所）。gh api→`gh-commit`（commit `committer.date`）／ http-feed→`feed-pubdate`（mode I I-3 が抽出し ingest にキャリー）／ 一般 URL・blog WebFetch→`html-meta`（構造化メタ）→ 取れなければ `html-body`（本文可視日付の保守パース・publication-context 限定・偽日付防止＝`claude.com/blog` の `Date:` を回収。`claude.com/sitemap.xml` は `lastmod` を持たないため sitemap 経路は claude.com で空振り＝dry-run 実証）／ Medium→`html-meta`（Phase 5 の Phase 2・`scrape-medium --emit-meta` が JSON-LD `SocialMediaPosting.datePublished`/`.dateModified` を正本に抽出した日付を出力先頭 frontmatter で受け取り転記。`--emit-meta` 非対応 minitools は `unknown` フォールバック）。取得不能・保守ルール非充足は `unknown`（代替しない）。各ページの代表鮮度日（`sources:` 全 raw の鮮度日の最大値＝各 raw は `last_modified` 優先・`unknown` は読み飛ばし・schema §3 定義）は index.md に `（鮮度: …）` で併記し、lint #17 と synthesize が再利用する。
...
- **lint**: 17 検査（機械判定＋意味解釈＋自動化停止監視＋ソース原本陳腐化 #17）。#11 のみ承認制で `## 矛盾` 末尾に決着注記を追記。検査の内訳は CLAUDE.md / `references/lint-rules.md` を参照。
...
| 未取り込み URL の発見 | **G** `discover-tier-a`（sitemap + CHANGELOG/README） | **I** `discover-watchlist`（RSS/Atom + Notion DB + 公式ブログ sitemap〔4c〕） |
| `current-baseline.md` version 系 | **自動更新可**（手動上書きも可） | **自動更新しない**（承認制＝決定6・W-4f 省略） |

Tier B が baseline を自動更新しない理由（決定6）: 再コンパイルで source ページの `claude_code_version` は進むため乖離は vault に**自己記述**され、`lint #3`（version 乖離）が次回対話で再検出して承認制の更新提案に合流する。よって「3 つ目の pending array」は不要（SKILL.md mode W が **決定 D3①** とラベルする判断＝§6.3）。
...
- 3 系統の plist は**起動時刻を stagger**（03:00 / 03:30 / 04:00）して同一 lock の競合 skip を回避。**4 系統目 plist は作らない**（mode I を流用＝D7・4c の公式ブログ sitemap も既存 discover-watchlist plist がそのまま拾う）。
- **cron 経路は decay しない取得手段に限定**（D3）: Tier A docs / Notion DB の API キー認証 / 公式ブログ公開 sitemap の curl（4c・auth 不要）のみ。Playwright auth decay を持つ Medium content fetch（4a）は**対話のみ**で、cron では発火しない（blog 記事の stage-2 WebFetch / ingest も対話のみ）。
...
> **ラベル体系は意図的に「現状のまま」記載**しています（正準化＝リナンバリングはしない）。CLAUDE.md / SKILL.md がこれらを現行ラベルで参照しているため、付け替えると新たな不整合を生むからです。ラベルは 4 系統が混在します: **ローマ字かな系**（基盤決定・spec 設計方針由来）／**数字系**（決定6・番号がずれている点に注意）／**D 系**（Phase 3f/3g/4 の brainstorm/steering 由来）／**ステップ系**（モード内手順・受け入れテスト発見事項）。各行の「出典」は深掘り先。
>
> **D 系ラベルは単一の連番ではなく、文書ごとに独立採番され同番号が再利用されている**点に注意。`.steering/` の各 phase（3f/4）が独自に D1〜 を振り直すため、**同じ D 番号が phase によって別の決定を指す**（例: D2 は 3f では「opt-in default-off」、Phase 4 では「raw は英語原文・翻訳済みを入れない」）。SKILL.md 内でも D3 が二重に使われる（baseline carrier 不要／cron は decay しない経路のみ）。下表は **SKILL.md / CLAUDE.md（公開文書）に現れる D ラベルの意味**を索引したもので、steering 作業文書の採番とは一致しない。「Phase」列は当該意味が確定した phase を示す。
...
### 6.3 D 系（SKILL.md / CLAUDE.md に現れる D ラベルの意味）

> 出典は SKILL.md の該当行。steering 作業文書（`.steering/2026053{1}-…-phase-3f/`, `.steering/20260606-…-phase-4/`）が一次根拠だが採番が phase ローカルのため、ここでは公開文書での意味を索引する。

| ID | 一言で | Phase | 出典 |
|----|--------|:---:|------|
| **D1** | registry-vs-flag → **flag**（別 registry を持たず source ページ frontmatter `watch:true`・URL は raw が正本） | 3f | SKILL §走査戦略（mode W）／ 3f steering |
| **D2** | opt-in **default-off**（`watch` 不在 or `false` は対象外） | 3f | SKILL mode W ／ 3f steering ※Phase 4 steering の D2 は別意（raw 英語原文） |
| **D3** | ①baseline carrier 不要（再コンパイルで `claude_code_version` が進み lint #3 が乖離を再検出＝新 pending array 不要）／②cron は **decay しない経路のみ**（Notion DB の API キー認証だけ・Playwright auth decay を cron に持ち込まない）。**SKILL.md 内で二重定義** | 3f／4 | SKILL mode W（①）／ SKILL I-3（②） |
| **D5** | retrofit 専用コマンドを新設しない（`watch:true` 手動追記で足りる・ingest 後の毎回 prompt は誤発火リスクで不採用） | 3f | SKILL retrofit |
| **D7** | anti-duplication（Notion-DB discovery〔4〕も公式ブログ sitemap discovery〔4c〕も新規 mode でなく **mode I 拡張**・`feed_registry[]` 経路・4 系統目 plist / 新 lint / 4 つ目 pending array は作らない） | 4／4c | Phase 4・4c steering ／ SKILL I-3 |
| **D8** | Medium URL は `--watch` 不可（mode W は WebFetch 既定で Playwright に乗れず毎晩 fail ＝ mode B reject ＋ mode W 防御 skip） | 4 | SKILL §5.3 |
| **D9** | tier と discovery 機構は**直交**。公式ブログ `claude.com/blog` は公式だが規範 docs ではない announcement のため **Tier B**（§4「任意ブログ = B」で充足・§4 改訂不要）。関連性が**不均質**なソースは関連性フィルタを持つ **mode I**（stage-1 keyword + stage-2 LLM）に載せる（mode G は同質ソース前提で filter なし）。Tier B 採用の副次効果として F-3 refresh 対象外（write-once blog の毎日再取得を回避）＝refresh 除外マーカー不要 | 4c | Phase 4c steering ／ CLAUDE.md #6 |
...
| **P5-1** | 出所は **per-date**（`published_at_source` / `last_modified_source`）。遡及補完で 2 日付が別経路から埋まるため単一 `date_source` を採らない | Phase 5 design ／ schema §3 |
| **P5-2** | 遡及補完 `--backfill-dates` は **ingest 側の一回限りバッチ**（mode B step 0.c）。lint は frontmatter を変更しない不変条件のため lint には置かない・ingest が raw frontmatter 書き込みの前例（schema §3 `source_url` 補完）・cron に載らず D7 はほぼ無関係 | Phase 5 design ／ SKILL mode B step 0.c |
| **P5-3** | b1（矛盾の日付補助）は **ingest 時注記に限定**（両 raw が手元・追加 Read 0）。#11 への日付軸追加はスコープ外（index がページ単位 1 日付しか持たず両側を区別できず追加 raw Read が要る）。Tier A 優先は不変＝日付で公式を覆さない | Phase 5 design ／ SKILL mode B step 7 |
| **P5-4** | lint #17 source-date-stale は **情報**（基盤記事のノイズ回避・#2 がコンパイル日鮮度を警告でカバー）。入力は index.md 代表鮮度日トークンの再利用で **追加 Read 0**・被覆は遡及補完後に効く | Phase 5 design ／ lint-rules #17 |
| **P5-5**（Phase 2） | Medium 日付は **minitools 先行改修**（`scrape-medium --emit-meta` が JSON-LD を正本に抽出・emit）に委ね、llm-wiki は出力先頭 frontmatter を**本文と分離して転記するだけ**。og `article:published_time` は更新日を指し誤誘導のため使わない（JSON-LD `SocialMediaPosting` が正本）。`--emit-meta` 非対応 minitools は出力全体を本文とみなし **unknown フォールバック**（偽日付を出さない clean degradation）。`--backfill-dates` は Medium 非対応（Playwright 再 ingest 必須）＝再 `ingest` で乗せる | Phase 5 の Phase 2 design ／ gate-finding ／ schema §3 `html-meta` |
...
| **5** | raw 元日付メタ（`published_at`/`last_modified`/per-date `*_source`・経路別抽出）＋ index 代表鮮度日（b4）＋ lint #17 source-date-stale（情報）＋ b1 ingest 矛盾注記 ＋ b3 synthesize 鮮度反映 ＋ `--backfill-dates` 一回限りバッチ | mode B step3/9・mode I I-3/I-5・lint を流用（新 mode/plist なし・backfill は ingest 側＝lint は frontmatter 不変更） |
| **5 の Phase 2** | Medium 日付抽出（mode B step 3 で `scrape-medium --emit-meta`・出力先頭 frontmatter を本文と分離し `html-meta` 日付を転記・notion 由来 K=5 ingest にも自動波及・`--emit-meta` 非対応は unknown フォールバック） | 5・minitools `scrape-medium --emit-meta` 先行マージ・gate-finding（JSON-LD 正本） |
