...
**日付メタ抽出（Phase 5・経路と直交）**: raw 確保時、同じ経路から**元ドキュメント日付**もベストエフォート抽出し raw frontmatter に記録する（`published_at` / `last_modified` ＋ per-date 出所）。gh api→`gh-commit`（commit `committer.date`）／ http-feed→`feed-pubdate`（mode I I-3 が抽出し ingest にキャリー）／ 一般 URL・blog WebFetch→`html-meta`（構造化メタ）→ 取れなければ `html-body`（本文可視日付の保守パース・publication-context 限定・偽日付防止＝`claude.com/blog` の `Date:` を回収。`claude.com/sitemap.xml` は `lastmod` を持たないため sitemap 経路は claude.com で空振り＝dry-run 実証）／ Medium→`unknown`（Phase 2）。取得不能・保守ルール非充足は `unknown`（代替しない）。各ページの代表鮮度日（`sources:` 全 raw の鮮度日の最大値＝各 raw は `last_modified` 優先・`unknown` は読み飛ばし・schema §3 定義）は index.md に `（鮮度: …）` で併記し、lint #17 と synthesize が再利用する。
...
| 4+（将来・deferred） | X 自動巡回（公開 RSS 無し・ToS グレー・gate 前提）・YouTube transcript・Medium 著者フィード・Medium 日付抽出（Phase 5 の Phase 2） | mode I http(s) feed として登録 |
