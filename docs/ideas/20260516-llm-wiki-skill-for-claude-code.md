...
| 3f | ウォッチリスト型 Tier B 定点観測（**単一 URL 型のみ**）— Tier B 既知 URL の日次自動 refresh | `refresh-tier-a` の **Tier B 版**（mode `refresh-watchlist` 新設）。単一 URL 型＝登録＝承認 → cron 自動 refresh（raw 再取得＋再コンパイル＋`## 矛盾` 自動追記まで無人・baseline 提案は次回対話＝決定6厳守）。relevance フィルタ不要（個別承認済 URL の refresh のみ）。取得可否 gate 不要（registration＝ingest 時に当該 URL を一度実取得済＝fetchability が per-URL で opt-in 時点に実証される）。**未決設計論点**: registry-vs-flag（別 registry か source ページ `watch: true` フラグか・連動して opt-in 既定 default-off/on も決定）・deferred baseline carrier（既存決定6 提案に相乗りできないか先に検討）。設計は次回 brainstorm/plan（2026-05-31 brainstorm で 3f/3g 分割・要件確定） |
...
- [ ] mode `refresh-watchlist` を新設する（mode F〔refresh-tier-a・Tier A〕は不可触。cron 非対話起動・`--dry-run` 副作用ゼロ）
- [ ] **単一 URL 型**: Tier B URL の ingest 時 opt-in 登録＝承認 → cron 自動 refresh（`refresh-tier-a` の Tier B 版・同一 WebFetch 経路）
- [ ] **無人範囲**: raw 再取得＋再コンパイル＋同一トピック `## 矛盾` 自動追記までは cron 無人。**baseline 提案は次回対話**（Tier B バージョン乗離は決定6＝承認制を厳守・自動上書きしない）
- [ ] deferred baseline 提案の durable carrier が定義され、次回対話 ingest/lint 起動時に AskUserQuestion で提示される（Tier A `migration_pending` フローに相当）。**※ 新 array を増やす前に検討**: `current-baseline.md` は既に `migration_pending`（301）と `pending_discoveries[]`（3c）を持つ。3 つ目の "pending" array は cognitive load・lint カバレッジ増。**まず既存の決定6 対話提案（ingest 時バージョン乖離検知→対話提案）に相乗りできないか**を design で検討し、専用 carrier 新設はそれが不可な場合の明示決定とする（default で `baseline_pending[]` を切らない）
- [ ] **【design 論点】registry-vs-flag**: 別途 watchlist registry を持つか、source ページに opt-in `watch: true` フラグを足すだけか（3e で source ページが正規化URLの正本＝registry 冗長の可能性。source_url＝既正規化キーで dedup/migration を回避）。当初要件の schema bump + watchlist 構造は継承せず設計時に再導出
- [ ] **【design 論点】opt-in 既定**: 「登録＝承認」の明示マーカー。default-off（source 毎 opt-in・一度きり記事を永久 refresh しない）vs default-on（全 Tier B を除外なき限り refresh）を決める。**※ 上の registry-vs-flag と連動**: flag-on-source-page を採れば default-off が唯一整合的（既存ページに無いフィールドを default-on にできない）、別 registry なら default-on も可能。矛盾ペアを選ばないよう同時決定する
- [ ] 書き込み mode 規約の再利用: 3a ロック規約・3d log.md pathspec 除外（`:!wiki/log.md`）・3b 空 commit ガード（再導出しない）。別 cron entry なら refresh-tier-a と同一 `.llm-wiki.lock` を争うため**起動時刻を stagger**（衝突 skip 回避）
- [ ] 取得可否 gate は**不要**（理由は「同一 WebFetch 経路」**ではなく**、**registration＝ingest 時にその URL を一度実取得済＝fetchability が per-URL で opt-in 時点に実証される**ため。任意 Tier B host〔Medium/個人ブログ/Substack 等〕は WebFetch hostile があり得るが、取れなかった URL はそもそも ingest できず watchlist に載らない。feed/X 固有の取得可否懸念は 3g の spike gate へ）
- [ ] **【design 論点・最優先】fetchability decay = 404/URL 消滅時の `refresh-watchlist` 挙動**: t=0 の実証は t=n を保証しない。Tier B は消滅率が Tier A 公式 docs より桁違いで daily refresh のドミナント運用障害。永続リトライ／stale フラグ付与／自動 opt-out 提案のいずれを採るか（mode F の Tier A 想定エラー扱いをそのまま流用できない）
- [ ] **lint #14 相当（`refresh-watchlist` 停止監視）**を #12/#13 同型で追加する（別 cron entry の停止検知。明示的省略でない限り必須）
- [ ] deferred baseline carrier の検討は **sync/async fork を先に潰す**: 「cron 再コンパイル時点で vault が乖離を自己記述する（lint 再検出できる）か」＝YES なら carrier 不要・lint 再検出に委譲、NO なら `migration_pending` クラスの durable carrier 新設（決定6 は同期提案・正しい先例は `migration_pending`）
- [ ] ingest 時 opt-in 登録の **UI/フロー**を定義（`--watch` 引数／ingest 後 AskUserQuestion／別コマンド後付けのいずれか。ingest の SKILL.md 変更範囲に直結）
- [ ] 論点5 **更新判定（raw 取得日 > `updated` で再コンパイル発動）**の 3f 継承を確認・走査対象集合の列挙方法（flag 方式なら `tier:B`＋`watch:true` のフロントマター Glob・件数増時のコンテキスト圧迫対処）を確定

### /llm-wiki Phase 3g（定点フィード Tier B・要件のみ記録／設計は次回 brainstorm・plan）

> 2026-05-31 brainstorm で 3f から分離。relevance フィルタ機構と承認モデルは下記で**確定済**（設計の残りは次回）。

- [ ] mode `discover-watchlist`（仮称・`discover-tier-a` の Tier B 版）を新設する（cron 非対話・`--dry-run` 副作用ゼロ）
- [ ] **定点フィード型**: サイト登録＝1回承認（購読モデル）→ 新着 URL を RSS/sitemap で発見
- [ ] **relevance フィルタ（確定: 2 段）**: a=タイトル/URL キーワード前段（cron 内・API コスト 0・粗ふるい）→ 通過分のみ b=軽量モデル判定後段で精査。非関連は negative cache（3e `declined` を再利用＝再候補化を防ぐ）
- [ ] **承認モデル（確定: 承認キュー経由・完全無人にしない）**: relevance 通過分も Tier A mode G と同じ capped バッチ opt-out surface に合流（無人 ingest しない）。Tier B バージョン乗離は決定6・矛盾は `## 矛盾` 自動追記で吸収・フィード単位の上限/除去で暴走防止
- [ ] **X 自動巡回は Phase 4 ソース別 fetcher 依存**として記録（X は公開 RSS 無し。当面は単一 URL 型〔3f〕に留め、自動巡回は RSS/sitemap 取得可能ソースに限定）
- [ ] 取得可否（特に非公式ソースの RSS/sitemap/fetcher）の最小 spike を**着手前 gate** にする（3a 同様）
