...
> 作成日: 2026-05-16
> ステータス: Phase 1 MVP verified（2026-05-22）/ Phase 2a verified（2026-05-23）/ Phase 2b verified（2026-05-23）/ Phase 3a implemented（2026-05-23・受け入れテスト pending）/ Phase 3b・3c・4 未着手
> 優先度: P1
...
### 将来対応予定
- Tier A（公式サイト/公式 GitHub）の既知 URL の日次自動再取得・再コンパイル・`current-baseline.md` 自動更新（Phase 3a・設計済み・実装待ち）
- session-start hook による自動コンテキストロード（Phase 3b・未設計）
- 会話中の URL 自動取り込み・overview 自動更新（Phase 3b・未設計）
- Tier A 公式 docs / 公式 GitHub の **未取り込み URL の自動発見＋初期登録**（Phase 3c・未設計。Phase 3a 設計時に切り出し。手動 `ingest` の初期登録コスト削減が目的。Phase 3a の lock 規約・migration_pending・schema フィールドを流用予定）
- ソース別取得ツール群（Phase 4）
