...
> 作成日: 2026-05-16
> ステータス: Phase 1 MVP verified（2026-05-22）/ Phase 2a verified（2026-05-23）/ Phase 2b verified（2026-05-23）/ Phase 3a verified（2026-05-24）/ Phase 3b・3c・4 未着手
> 優先度: P1
...
- **Phase 3a 受け入れテスト発見事項 F-1〜F-5 の Phase 3b 取り扱い**:
  - **F-1（最優先）**: SKILL.md F-4a に **github.com URL → `gh api` 経路への routing** を追加。現状 GitHub blob URL は WebFetch では本文取得不能で `claude-code-changelog-verbatim` 系の Tier A ソースが毎回 fail し続けるため、Phase 3b で最優先解消。
  ...
  - **F-4（重要）**: migration_pending 承認後も refresh が old URL で取得を継続する設計ギャップ。SKILL.md §2.5 (i) を「URL 書き換え」から「new_url で新規 raw を ingest して source ページ更新」に再定義（F-1 と合わせて取り扱う妥当性が高い）。
