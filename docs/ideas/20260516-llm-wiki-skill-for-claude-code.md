...
> 作成日: 2026-05-16
> ステータス: Phase 1 MVP verified（2026-05-22）/ Phase 2a verified（2026-05-23）/ Phase 2b verified（2026-05-23）/ Phase 3a verified（2026-05-24・F-1 hotfix 2026-05-24）/ Phase 3b verified（2026-05-27）/ Phase 3d verified（2026-05-29・mode B step 3.5(i) の source_url 解決バグを 2026-05-30 に発見し PR #14 で是正済）/ Phase 3c verified（2026-05-30・本体 PR #13 merged・schema v1.4.1 patch は PR #12 merged・carve-out 実機検証で発見した G-4 / step 3.5(i) source_url 解決バグは PR #14 `c9d8843` で是正済）/ Phase 3e verified（2026-05-31・会話 URL hook + mode H review + URL 正規化フル仕様 + declined negative cache + mode G opt-out amend・実機受け入れテスト 6 項目 PASS）/ Phase 3f verified（2026-06-05・単一 URL watchlist・mode W refresh-watchlist + watch/fetch_status + lint #14/#15 + schema v1.7.0・PR #21 `9d7e299`→main `db39199`）/ Phase 3g verified（2026-06-05・定点フィード Tier B・mode I discover-watchlist + feed_url + pending_feed_discoveries + lint #16 + schema v1.8.0・PR #23 `57e3948`→main `d98142e`）/ Phase 4 verified（2026-06-06・X deferred・Medium 2 層構造＝4a per-host content routing〔mode B step 3 medium 分岐・`scrape-medium --cdp`・英語原文 raw・対話のみ〕+ 4b Notion-DB-as-discovery〔mode I I-3 を feed_registry 経路で拡張・`discover-notion-medium` cron・stage-2 は summary 再利用・K=5 cap〕・schema v1.9.0〔feed_registry[]・fetched_via minitools-playwright・denylist source・pending_feed_discoveries.summary〕・minitools_path 設定・mode B `--feed=notion-*:` registry append 特例・Medium watch reject〔D8〕・スペック検証 38 条件 + 実機 carve-out 8 項目すべて PASS〔実 vault/minitools/Chrome で実施・`scrape-medium --cdp` は input() を持つため非対話は `printf '\n'｜` で stdin 供給が必要と判明〕）/ Phase 4c verified（2026-06-12・遡及検証・`claude.com/blog` 定点観測＝mode I I-3 sitemap 経路・Tier B・新モード/lint/plist なし〔D7〕・schema v1.10.0・PR #28 `6ec2779`→main `5a4ad59`・スペック検証 13 条件 PASS ＋ 実 vault で `discover-watchlist` 複数回実走〔sitemap 144 英語 blog → ~71 本 tier:B ingest・stage-1/2 フィルタ・cap 除外・BSD sed/phantom append の実バグ 2 件を実走中に是正〕）/ Phase 5 verified（2026-06-12・raw 元日付メタ＝`published_at`/`last_modified` ＋ per-date 出所・mode B/F/I 経路別抽出〔gh-commit / feed-pubdate / sitemap-lastmod / html-meta / html-body〕・`--backfill-dates` 一回限りバッチ・lint #17 source-date-stale・index 代表鮮度日・schema v1.11.0・PR #32 `1d177fd`→main `708db73`・スペック検証 23 条件 PASS ＋ 実 vault で `--backfill-dates` 実走〔scanned=237・該当経路 unknown 0・index 鮮度トークン 160 件〕。Medium 日付は Phase 2 ゲートで据え置き）
> 優先度: P1
...
cd ~/Projects/claude-code-wiki-maker
...
| 4 | ソース別取得ツール（**Medium 2 層構造に再スコープ・X deferred**） | **プラットフォーム崩し（2026-06-06 brainstorm）**: 当初の「X / Medium / Notion / 公式サイト」均等 4 fetcher は誤スコープと判明。Medium は既存コードベース `/Users/tak/Projects/minitools/` の Playwright CDP scraper ＋ Notion Medium DB（cron が 200+ claps を毎日保存）を流用、Notion/公式サイト/YouTube は既存ツール（MCP・RSS・WebFetch）でほぼ充足、**X のみ本質的難問（公開 RSS 無し・ToS グレー）＝ deferred**。**確定 2 層構造**: **4a = per-host content routing**（mode B step 3 raw 確保に host 別分岐を追加・F-1 hotfix〔github.com→gh api〕の先例を踏襲・`medium.com` → minitools `scrape-medium --cdp` で英語原文 Markdown → raw・**対話のみ**＝Playwright ログインセッション decay を cron 経路に入れない）／**4b = Notion-DB discovery（mode I `discover-watchlist` を拡張）**（advisor レビューで mode 新設→**mode I 拡張**に決定〔D7・anti-duplication〕＝I-3 に走査経路を 1 本追加し Notion DB を discovery source の一種〔`current-baseline.md.feed_registry[]` の `notion-medium-db` エントリ〕として食わせる・I-4〜I-8/lint #16/cron 04:00/heartbeat/pending_feed_discoveries を流用＝4 つ目 pending array・lint #17・4 系統目 plist は不要・cron が `discover-notion-medium` で新着取得 → keyword フィルタ → capped opt-out 承認 → mode B ingest → 4a routing で Playwright 取得〔K=5 cap〕）。**raw は英語原文を保存**（Notion 翻訳済みは Claude Code 関連で未翻訳分を含むため・verbatim 原則厳守＝URL から Playwright で英語再取得）。**advisor 指摘で 2 件確定**: (D8) **Medium URL は `--watch` 不可**（mode W は WebFetch 既定で step 3 routing を通らず Playwright に乗れない＝毎晩 fail するため mode B で reject）、(§4.7) discovery window の date semantics は **実コード確認で解決済**〔当初「Notion `Date`=公開日」と懸念したが、実際は `target_date`=Daily Digest 取得日であり公開日フィールドは存在しない〔Gmail digest では公開日不明〕＝`--days N` は取得日フィルタで late-added 漏れなし・**minitools created_time フィルタ追加は不要**〕。minitools 依存基盤（`scrape-medium`/`discover-notion-medium` 2 CLI）は 2026-06-06 実装・実機検証済。X 自動巡回は **Phase 3g からの sequencing inversion** で本 Phase に持ち越したが ToS 確認・取得 spike 前のため deferred 継続。設計詳細: `.steering/20260606-llm-wiki-phase-4/` |
...
### /llm-wiki Phase 3g（定点フィード Tier B・**verified 2026-06-05**・PR #23 `57e3948`→main `d98142e`）
...
- 複数エージェント同時書き込みの競合解決
- ソース別取得ツール（**Medium 2 層構造に再スコープ・X deferred**）→ Phase 4（2026-06-06 brainstorm でプラットフォーム崩し。Notion/公式サイト/YouTube は既存ツールで充足・X は ToS グレーで deferred）
...
- ソース別取得ツール群・**Medium 取り込み**（**Phase 4・brainstorm 完了 2026-06-06／設計着手**）。**プラットフォーム崩し**: 当初の均等 4 fetcher（X/Medium/Notion/公式）は誤スコープ＝Notion は `mcp__claude_ai_Notion__*` 既存・公式サイトは WebFetch・YouTube は RSS〔`feeds/videos.xml`〕で 3g 流用可・**X のみ本質的難問**（公開 RSS 無し・スクレイピング ToS グレー）。**確定 2 層構造**（既存コードベース `/Users/tak/Projects/minitools/` 流用・**設計完了 `.steering/20260606-llm-wiki-phase-4/`**）: **(4a) per-host content routing**＝mode B step 3（raw 確保）に host 別分岐追加・**F-1 hotfix〔github.com→gh api〕の先例を踏襲**・`medium.com` → minitools `scrape-medium --cdp`（Playwright CDP・英語原文 Markdown → raw）・**対話のみ**（Playwright ログインセッション decay を cron 経路に持ち込まない＝3f fetchability-decay 決定を auth decay に適用）。**(4b) Notion-DB discovery = mode I `discover-watchlist` を拡張**（**advisor レビューで mode 新設→mode I 拡張に決定〔D7・anti-duplication〕**＝fork は I-3〔フィード取得手段〕だけで I-4〜I-8/lint #16/cron 04:00/heartbeat/pending_feed_discoveries は同一＝**document-less な Notion DB は `current-baseline.md.feed_registry[]` に登録**し I-3 が source ページ feed_url〔http feed〕に加えて feed_registry も走査する〔`feed_url` 値域は不変・raw-less source ページ衝突を回避〕・**4 つ目 pending array・lint #17・4 系統目 plist は不要**）・cron が minitools `discover-notion-medium`（Notion Medium DB＝minitools cron が 200+ claps を毎日保存）で新着取得 → keyword フィルタ → capped opt-out 承認 → mode B ingest → 4a routing で Playwright 英語取得（**K=5 cap**＝sequential Playwright の 2 分待ち tail 制限）。**raw は英語原文を保存**（Notion 翻訳済みは Claude Code 関連で未翻訳分を含むため不採用・verbatim 原則厳守＝URL から再取得）＝[[webfetch-raw-snapshot-policy]] と整合。**advisor 指摘で確定 2 件**: (D8) **Medium URL は `--watch` 不可**（mode W は WebFetch 既定で step 3 routing を通らず Playwright に乗れない＝毎晩 fail するため mode B で reject + mode W 防御 skip）、(§4.7) **discovery window の date semantics は実コード確認で解決済**＝当初「`discover-notion-medium` は Medium 公開日でフィルタ → late-added 漏れ」と懸念したが、`scripts/medium.py` line 195 で Notion `Date` = `target_date`（Daily Digest 取得日）と確定〔`Article` に公開日フィールドなし・Gmail digest では公開日不明〕＝取得日フィルタなので late-added 漏れなし・**minitools created_time フィルタ追加は不要**（ユーザー指摘で「Date 名 = 公開日」の外部仮定を実コードで是正）。**minitools 依存基盤（`scrape-medium`/`discover-notion-medium` 2 CLI）は 2026-06-06 実装・実機検証済**（`docs/minitools-phase4-instructions.md`・stdout 汚染を `_redirect_logging_to_stderr` で解消）。**schema v1.9.0**（§2.1 に `feed_registry[]`〔document-less discovery source 登録・pending array ではない〕追加・`feed_url` 値域は不変・`fetched_via: minitools-playwright`・URL denylist `source` 追加）。**X deferred**（ToS appetite・取得 spike を着手前 gate に・判断は設計後）。実装前ゲートは全クリア・次作業: T1 schema から実装着手
...
```
claude-code-wiki-maker/        # 個人 Claude Code 知識ハブ専用リポジトリ（スキル資産の置き場）
...
- 2026-06-05: **Phase 3g 設計・実装**（定点フィード型 Tier B・mode I `discover-watchlist`）。`.steering/20260605-llm-wiki-phase-3g/` の design.md（14 章・mode I step I-1〜I-8・spike GATE §2・queue cap §4.2・stage-2 統合 §4.4）/ requirements.md（A〜K 群）/ tasklist.md（T0〜T9）に基づき実装。**取得可否 spike GATE PASS（2026-06-05）**: simonwillison.net / oneusefulthing.org / huyenchip.com 3 サイトで curl RSS/Atom 取得 + entry URL 抽出 + stage-1 keyword フィルタを実証。**schema v1.7.0 → v1.8.0**（§2 共通フィールドに任意 `feed_url`〔url 文字列・`--feed` mode B が立てる・mode I 走査対象マーカー〕、§2.1 に `last_discover_watchlist_run`〔run heartbeat・version baseline ではない〕と `pending_feed_discoveries[]`〔mode I 専用 Tier B フィード発見 URL 保留配列・`declined_reason` 3 値含む〕、編集境界表に mode I 列・additive minor）／ `.llm-wiki.json` 1.8.0。**SKILL.md**: mode B に `--feed=<rss_url>` 引数（source ページに `feed_url` を立てる・`--watch` と併用可・**`--feed` 非伝播**〔mode G/H/I/F migration は渡さない〕・retrofit は frontmatter 手動追記）／ **mode I 新設**（I-1 lock 〜 I-8 サマリ+解放・I-3 フィード巡回〔curl 逐語 fetch・WebFetch 不使用・RSS/Atom 両形式 bash 抽出〕・I-4 突合+stage-1 keyword フィルタ〔hardcoded keyword セット・LLM コスト 0〕・I-5 `pending_feed_discoveries` append+cap/eviction〔per-feed N=50・全体 M=200〕・I-6 capped バッチ opt-out+stage-2 LLM 統合〔WebFetch JSON relevance 判定・confidence<0.7 → auto-decline〕・I-7 heartbeat・`--no-prompt`〔stage-1+append のみ・stage-2 不実行〕・`--dry-run`・エラー表）／ ステップ0/0.6 分岐・lock mode 値に `discover-watchlist`／ mode L を 15 → **16 検査**（#16 追加・`--check` キー・log.md サマリ文字列・書き込み境界表）／ グローバルエラーディスパッチ表に mode I 行。**lint-rules.md**: #16 `last-discover-watchlist-run`（既定 14 日・#13 同型・追加 Read 0 回）／ 検査表 16 化・しきい値表・走査戦略 Phase 3g 追加 0 回・fixture カタログ #16・「40 ケース」。**plist**: `references/discover-watchlist-launchd.plist.example`（refresh-watchlist 同型・**起動 04:00 で 3 系統 stagger**〔03:00/03:30/04:00〕・`--no-prompt` 起動・5 環境変数）。`feed_url` / `pending_feed_discoveries[]` / `declined_reason` discriminator は schema.md §2/§2.1 が単一正本（SKILL.md で再定義せず）。idea.md Phase 3g 受け入れ条件を [x] 化・ステータス行・CLAUDE.md ステータス/主役機能表/ロードマップ・README 含まれるもの表/推奨ワークフローを更新。動作確認は方針 (B) スペック検証。実機 deferred 8 項目（`ingest --feed` 登録通し・mode I `--no-prompt` cron 通し・stage-2 LLM 判定通し・declined キャッシュ動作・lint #16 停止監視・cron 3 系統 stagger 実機・`--feed` 非伝播・queue cap eviction 実機）は requirements §carve-out として受け入れテスト時の利用者裁量。vault co-evolution（current-baseline.md ポインタ v1.8.0 + log.md 改訂行）は実装 tail で実施。**PR #23 `57e3948`→main `d98142e`**。
- 2026-06-06: **Phase 4 開始 brainstorm を実施し、X / Medium / Notion / 公式サイトの均等 4 fetcher から「Medium 2 層構造・X deferred」へ再スコープ**。advisor レビューで**プラットフォーム崩し**を実施＝(1) Notion は `mcp__claude_ai_Notion__*` が既存ツールで充足、(2) 公式サイトは WebFetch で充足、(3) YouTube は RSS〔`youtube.com/feeds/videos.xml`〕で 3g 流用可、(4) **X のみ本質的難問**（公開 RSS 無し・スクレイピング ToS グレー・API 有料）と判明。**Phase 4 の実質課題は「X をどうするか」ではなく「Medium をどう取るか」**に収束。**ユーザー提供の既存コードベース `/Users/tak/Projects/minitools/`** を調査し 2 つの取得経路を確認: (a) `scrapers/medium_scraper.py` の **Playwright CDP scraper**（Cloudflare 回避・Medium ログインセッション利用・初回のみ対話ログイン）、(b) `readers/notion.py` ＋ minitools cron が **200+ claps の Medium 記事を毎日 Notion DB に保存**。**advisor 再フレーミングで 2 経路は競合でなく補完と確定**: Playwright=対話 on-demand（mode B/H・セッション decay は人間が対応）／Notion-DB=cron 自動（API キー認証のみ＝セッション decay 回避）。**ユーザー確定事項**: (1) X は一旦スキップ・他から着手、(2) X の ToS appetite は設計後に判断、(3) Notion Medium DB は minitools cron で毎日新着が溜まっている、(4) **raw は英語原文を保存**（Notion 翻訳済みは Claude Code 関連記事で未翻訳分を含むため・Jina AI Reader は Medium 側が拒否で使えず＝**Playwright で URL から英語再取得**が必須）、(5) minitools のライブラリ化は llm-wiki スコープでは不要（Bash 経由呼び出しは CLI/lib 問わず同じ）＝**専用 CLI エントリーポイント 2 本を minitools に追加**する方が実用的。**確定 2 層構造**: **4a = per-host content routing**（mode B step 3 raw 確保に host 別分岐・**F-1 hotfix〔github.com→gh api〕の先例踏襲**・`medium.com` → `scrape-medium --cdp`・対話のみ）／**4b = Notion-DB-as-discovery**（mode I 同型・cron → `discover-notion-medium` → keyword フィルタ → capped バッチ opt-out 承認 → mode B ingest → 4a routing）。**minitools 依存基盤を先行実装（別セッション・2026-06-06 実機検証済）**: `scripts/scrape_medium.py`（`MediumScraper`+`MarkdownConverter`・英語 Markdown stdout・CDP で 34KB 出力確認）／`scripts/discover_notion_medium.py`（`NotionReader`・直近 N 日 JSON stdout・7 日で 72 件確認）／`pyproject.toml` に 2 scripts 追加。**実装中の発見**: minitools 各モジュールが import 時に `get_logger()` で stdout ハンドラを直付けするため `setup_logger()`/root basicConfig では stdout 汚染を抑制不能 → 両スクリプトに `_redirect_logging_to_stderr()` を追加して stdout をクリーン化（llm-wiki から Bash 呼び出し時の前提を担保）。指示書は `docs/minitools-phase4-instructions.md`。**残 design 論点**（次回 `.steering/` で決着）: minitools パスの設定方法（`.llm-wiki.json` か env）・fetcher availability check（gh 同型 clean failure＝Medium 取得失敗は skip+lint surface）・`pending_medium_discoveries[]` 置き場（4 つ目 pending array か）・lint #17（`last_discover_medium_run` 停止監視・#16 同型）・schema v1.9.0 bump・raw frontmatter `fetched_via: minitools-playwright` ＋ verbatim 原則注記。idea.md ステータス行・ロードマップ表 Phase 4 行・スコープ外・将来対応予定 Phase 4 行を更新。次作業: `.steering/20260606-llm-wiki-phase-4/` design.md/requirements.md/tasklist.md 起こし（X deferred は ToS spike を着手前 gate に）。**→ 改訂（同日 `.steering/` 起こし時・advisor レビュー）: (1) 4b を mode 新設 → mode I 拡張に決定（D7）＝`pending_medium_discoveries[]`（4 つ目 array）・lint #17・4 系統目 plist はすべて撤回（pending_feed_discoveries / lint #16 / cron 04:00 / heartbeat を流用）。(2) Medium URL は `--watch` 不可（D8・mode W が Playwright に乗れず毎晩 fail）。(3) discovery window の date semantics（公開日フィルタが late-added を漏らす・minitools created_time フィルタ追加が本命・要ユーザー確認）。(4) document-less な Notion 登録は `wiki/sources/` ページ（raw-less）にすると lint sources-empty / 不変条件 #2 / mode B pipeline と衝突するため `current-baseline.md.feed_registry[]` に置く（schema §2.1 追加・`feed_url` 値域は不変）。(5) URL denylist に `source`・Playwright バッチ ingest K=5 cap。設計 3 点セット完成。**
**→ 再改訂（同日・ユーザー指摘で実コード確認）: (3) date semantics は撤回＝Notion `Date` は Medium 公開日でなく Daily Digest 取得日（`scripts/medium.py` line 195 `target_date`・`Article` に公開日フィールドなし・Gmail digest では公開日不明）と確定。`--days N` は取得日フィルタで late-added 漏れは発生せず、minitools の created_time フィルタ追加は不要（T9 撤回）。「Date という名前 = 公開日」は外部仮定で、実コードで是正。実装前ゲートは全クリア。(4) の document-less 登録衝突は `current-baseline.md.feed_registry[]` で解決（raw-less source ページを作らないことで lint sources-empty / 不変条件 #2 / mode B pipeline の 3 衝突を回避）。I-3 早期 return guard は「feed_url 0 件 AND feed_registry 空」両方真に改修（mode I 非破壊の唯一の例外）。**
- 2026-06-06: **Phase 4 設計・実装**（Medium 取り込み・2 層構造 / X deferred）。`.steering/20260606-llm-wiki-phase-4/` の design.md（13 章・4a per-host content routing §5.1 / 4b mode I I-3 ソース別分岐 §5.2 / feed_registry §4.5 / K=5 cap §4.6 / date semantics §4.7 / denylist source §4.8）/ requirements.md（A〜I 群 + carve-out）/ tasklist.md（T0〜T10）に基づき実装。**依存基盤 GATE PASS**（minitools 2 CLI 実装・実機検証済＝T0）。**schema v1.8.0 → v1.9.0**（§2.1 に `feed_registry[]`〔document-less discovery source 登録・`{source, selector, label, added}`・**pending array ではない**＝pending は 3 つのまま〕追加・編集境界表 feed_registry 行〔mode B `--feed=notion-*:` が書き mode I I-3 が読む〕／ `pending_feed_discoveries[]` に任意 `summary`〔notion-medium-db 由来のみ・stage-2 で WebFetch を Medium に当てない〕追加／ §4 `fetched_via` 語彙に `minitools-playwright`〔Tier B・verbatim 注記〕／ URL 正規化 denylist に `source`／ **`feed_url` 値域は不変**・additive minor）／ `.llm-wiki.json` 1.9.0 + `minitools_path`。**SKILL.md**: ステップ0.5 に minitools_path 解決〔未設定/不在は Medium 機能のみ無効化・clean failure〕／ mode A init で minitools_path 対話設定／ mode B step 3 を host routing 化〔`medium.com`/`*.medium.com` → `scrape-medium --cdp` → 英語原文 raw〔`fetched_via: minitools-playwright`〕・exit 1 で当該 source 中断・availability check〕／ mode B step 0.a に `--feed=notion-*:` registry append 特例〔ingest short-circuit・feed_registry append・1 commit・重複 no-op〕／ step 0 ガード緩和〔`--feed=notion-*:` は path-or-url 省略可〕／ mode B step 6 に **Medium + `--watch` reject**〔D8〕／ **mode I I-3 を 2 経路に拡張**〔(1) http(s) feed_url curl〔不変〕/ (2) feed_registry の notion-medium-db → `discover-notion-medium`〔selector=default は `--database-id` 省略〕→ JSON パース・source 種別タグ〕・**早期 return guard 改修**〔feed_url 0 件 AND feed_registry 空の両方真でのみ early return〕／ I-4 stage-1 に Notion entry〔title/japanese_title/summary/url〕／ I-5 で notion entry の `summary` 永続化／ I-6 stage-2 をソース種別で条件分岐〔notion は summary 入力で WebFetch 省く・http-feed は従来 WebFetch〕＋ **ingest cap K=5**〔Playwright sequential のみ・提示 N=20 と http-feed ingest は非 cap＝mode I 非破壊〕／ mode W に Medium 防御 skip〔§5.3・lint #15 を汚さない〕／ グローバル & mode B/I エラー表に Medium 行。**lint**: **新規検査なし**（mode L は 16 検査のまま・#16 が Notion source の cron 停止も heartbeat 共有で surface）。**plist**: `references/discover-watchlist-launchd.plist.example` の env に `NOTION_API_KEY` 追記（4 系統目 plist は作らない・Chrome/Playwright env 不要＝cron は Playwright 不発火）。idea.md ステータス行・CLAUDE.md ステータス/主役機能表/ロードマップ/不変条件 5,6・README 含まれるもの表/推奨ワークフローを更新。動作確認は方針 (B) スペック検証（requirements 検証手順 1〜5＝スペック整合・3 面整合・不変条件・clean failure・mode I 非破壊）。**advisor 指摘 4 件を反映**: (#1) Notion summary を pending エントリに永続化〔aged entry が Medium へ WebFetch しない〕、(#2) `--feed=notion-*:` の path-or-url 省略を step 0 で許可、(#3) selector=default は `--database-id` 省略〔env 解決〕、(#4) K=5 は Notion/Playwright scoped〔http-feed ingest を退行させない〕。実機 carve-out 8 項目（4a 通し・4b 登録/cron/対話通し・D8 watch reject・mode I 非破壊・minitools 不在 clean failure・`--dry-run` 副作用ゼロ）は requirements §carve-out として受け入れテスト時の利用者裁量。vault co-evolution（current-baseline.md ポインタ v1.9.0 + log.md 改訂行）は実装 tail で実施。

> **Phase 4 を 2 層構造（Medium）に再スコープした経緯（2026-06-06）**: 当初「ソース別取得ツール（X / Medium / Notion / 公式サイト等）」の均等 4 fetcher だったが、プラットフォーム崩しで Notion（MCP 既存）/公式サイト（WebFetch）/YouTube（RSS）は既存ツールで充足・X のみ本質的難問（公開 RSS 無し・ToS グレー）と判明し、**実質課題は Medium 取得**に収束。既存コードベース minitools の Playwright CDP scraper（対話 content fetch）と Notion Medium DB（cron が 200+ claps を curate・discovery source）を流用する 2 層構造（4a per-host content routing / 4b mode I 拡張）に確定。**advisor レビューで mode 新設→mode I 拡張に倒し**（fork は I-3 のみ・anti-duplication）、raw-less 登録ページ衝突を feed_registry で回避。X は deferred（ToS spike を着手前 gate に）。詳細: `.steering/20260606-llm-wiki-phase-4/`。
