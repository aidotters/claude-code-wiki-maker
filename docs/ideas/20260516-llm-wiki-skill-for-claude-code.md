...
> 作成日: 2026-05-16
> ステータス: Phase 1 MVP verified（2026-05-22）/ Phase 2a verified（2026-05-23）/ Phase 2b verified（2026-05-23）/ Phase 3a verified（2026-05-24・F-1 hotfix 2026-05-24）/ Phase 3b verified（2026-05-27）/ Phase 3d brainstorm 完了（2026-05-29）/ Phase 3c・3e・4 未着手
> 優先度: P1
...
- [ ] SKILL.md mode B（ingest）が「migration_pending 承認後 ingest」を内包する（専用モード追加なし・既存 ingest フローを拡張）
- [ ] SKILL.md §2.5 (i) `migration_pending` 提案フローが「URL 書き換え」から「new_url で新規 raw を ingest して既存 source ページを更新（sources: append、source_url を new_url に書き換え、migration_pending エントリ削除、updated 進行、title 等の手動編集領域は不可触）」に再定義される
- [ ] 同一 source 判定ロジックが SKILL.md mode B に明文化される: (i) 正規化後 source_url 完全一致 → 既存 source ページに統合 / (ii) `current-baseline.md.migration_pending[].new_url` と一致 → 旧 source ページに統合＋ migration_pending エントリ削除 / (iii) どちらでもなければ新規 source ページ作成
- [ ] URL 正規化規約（3d 用最小ルール）: lowercase host ＋末尾スラッシュ除去のみが SKILL.md mode B に明文化される。フル仕様（tracking param 除去・フラグメント除去・Tier A allowlist 等）は Phase 3e で再開する旨を脚注で明記
- [ ] Tier A 自動判定は 3d では不要（migration_pending 由来は元 source の Tier をそのまま継承）。SKILL.md / schema.md に host allowlist を持たない
- [ ] migration case の旧 raw（old_url）は不変保持（決定 E1 整合）、新 raw のみ `sources:` 末尾に append（時系列保証）
- [ ] 矛盾検出は既存 ingest 規約踏襲（同一トピック [[wikilink]] 先のみ即時照合、矛盾は `## 矛盾` セクション末尾に自動追記）
- [ ] migration case ingest は `.llm-wiki.lock` を取得（書き込みモード）、ソース単位で git commit、`git push` はしない
- [ ] F-6: SKILL.md mode B / mode F 該当箇所に「再コンパイル時、新 raw を source ページの `sources:` 末尾に append（時系列保証）」が明文化される（既存実装の仕様化のみ）

**overview 自動更新（C）**:
- [ ] vault `wiki/overview.md` に `## 現状` セクション（agent 完全所有・自動更新領域）が定義され、上部の説明文は手動編集領域として保持される
- [ ] `## 現状` セクションは統計値（ソース数 / concept 数 / synthesis 数 / practice 数 / feature 数）・最終 ingest 日付・最終更新日付（YYYY-MM-DD）を保持する
- [ ] overview 更新は書き込みモード操作（mode B ingest / mode F refresh-tier-a / mode D synthesize）の同一 commit 内で inline 実行される（別 commit にしない）
- [ ] 値変化ガード付き（同値なら Edit / commit skip ＋ log に `overview unchanged` 行追記）— Phase 3b F-5 と同じ流儀
- [ ] lint #11 承認制決着では overview を更新しない（source ページ統計に変化なし）
- [ ] schema.md に overview.md の構造（agent 所有領域 vs 手動編集領域・統計フィールドの定義）が明記される
- [ ] 統計値の取得は SKILL.md mode B に明文化（`Glob` で wiki/<type>/*.md を count、index.md 1 回読みは不要）

**F-3 log.md append 規約見直し（(iv) F-2 dirty check から log.md 除外）**:
- [ ] SKILL.md mode F の F-2 dirty-state 判定式が `git -C wiki-vault status --porcelain -- ':!wiki/log.md'` 非空に変更される（log.md は agent 完全所有領域のため dirty check 対象から除外）
- [ ] F-2 skip 時の log.md append + commit が成功するようになり、dirty escalation ループが解消される（既存の skip + log append 動線はそのまま、commit が新たに通る）
- [ ] schema.md §3 / SKILL.md に「log.md は agent 完全所有・追記のみ規約のため dirty 状態でも append + commit 可」が明記される
- [ ] F-3 と F-4 を旧フレーミング（2026-05-24）で「同じ log.md append 規約として束ねた」点が訂正済と SKILL.md / idea.md で読み取れる（F-3 は cron dirty 特殊事象・F-4 は通常 ingest と独立論点）

**schema co-evolution / idea.md**:
- [ ] schema.md の `schema_version` を v1.3.0 → v1.4.0 に bump（overview.md 構造化セクション追加・dirty check 規約改訂）
- [ ] `current-baseline.md` フロントマターの `schema` ポインタを更新（ボールト側 log.md にも 1 行追記）
- [ ] idea.md 更新履歴に Phase 3d 実装エントリが追加される
- [ ] CLAUDE.md ステータス行（"MVP（Phase 1）＋…実装済み"）が Phase 3d 実装完了時点で更新される

**実機反映（vault co-evolution）**:
- [ ] vault `wiki/overview.md` の初回構造変換 commit が落ちる（既存内容を `## 現状` セクション形式へ移行、agent 所有領域の初回反映）
- [ ] vault `wiki/current-baseline.md` の `schema` ポインタ更新 commit が落ちる
- [ ] SKILL.md / schema.md の repo A 編集と vault co-evolution が同一 PR 範囲内で完結する
...
- Tier A 公式 docs / 公式 GitHub の **未取り込み URL の自動発見＋初期登録**（Phase 3c・未設計。Phase 3a 設計時に切り出し。手動 `ingest` の初期登録コスト削減が目的。Phase 3a の lock 規約・migration_pending・schema フィールドを流用予定。**Phase 3d で確立される共通 surface（mode B 拡張）をそのまま流用して cron 自動 ingest を実装可能**。Phase 3d 完了後に着手する想定）
- ソース別取得ツール群（Phase 4）
