...
| 3f | ウォッチリスト型 Tier B 定点観測（B の残り）— 単一 URL 型＋定点フィード型の日次自動 refresh / discover | `refresh-tier-a`/`discover-tier-a` の **Tier B 版**。単一 URL 型＝登録＝承認→自動 refresh、定点フィード型＝**サイト登録＝1回承認→新着は cron 自動 ingest**（購読モデル）。**定点フィード自動 ingest には Claude Code relevance フィルタ必須**（判定機構は 3f 設計で確定）。X は公開 RSS 無し＝**Phase 4 ソース別 fetcher 依存**のため当面は単一 URL 型に留め自動巡回は RSS/sitemap 取得可能ソースに限定。取得可否 spike を着手前 gate に（要件のみ記録・設計は次回 brainstorm/plan） |
| 4 | ソース別取得ツール（X / Medium / Notion / 公式サイト等） | 取り込み拡充。**Phase 3f 定点フィード（特に X）の前提依存**（sequencing inversion: 3f-feed は Phase 4 fetcher と前後して着地） |
...
**A 群: 会話 URL hook（検出器）**
- [ ] UserPromptSubmit hook 設定例が `references/` に同梱される（`session-start-hook.example.json` と同流儀・利用者が `.claude/settings.json` に手動マージ・project local 推奨・POSIX shell 互換）
- [ ] hook は会話中の URL を正規表現で検出し、**追記専用 inbox ファイルに append するだけ**（スキル起動なし・lock 取得なし・公式 docs `hooks.md`/`skills.md` で hook がスキル起動不可・additionalContext/block/シェル実行のみと実証済）
- [ ] hook の URL フィルタ（localhost・コード例・既取り込み `source_url`・既キュー `pending_discoveries` の除外）が定義される

**B 群: inbox → キュー → 承認 → ingest**
- [ ] inbox ファイルの所在（vault 内・append-only・`log.md` 同様 agent 完全所有）と path が定義される（vault 内/外の最終判断を含む）
- [ ] inbox path が `refresh-tier-a` F-2 dirty-check の pathspec 除外（`:!...`）に追加される（捕捉 URL が夜間 refresh を silent block しない）
- [ ] inbox path が `discover-tier-a`（mode G）の dirty-check 除外にも追加される
- [ ] 対話時に inbox を `pending_discoveries[]` に吸い上げる経路が定義される
- [ ] Tier B 単一 URL（会話 URL 含む）は **opt-in 個別承認**（`AskUserQuestion`）→ 承認分のみ共通 surface（mode B）で ingest
- [ ] 会話 URL の**承認 surface が確定する**（`discover-tier-a` がキュー全体を提示するか、専用 review surface（`/llm-wiki review` 的）を新設するか — 設計で決定）

**C 群: URL 正規化フル仕様（parked・line 349 / 旧 3d 送り）**
- [ ] 正規化ルールが 3d 最小（lowercase host ＋末尾スラッシュ除去）から**フル仕様**（tracking param 除去・fragment 除去・Tier A allowlist 等）に拡張され schema / SKILL.md に明文化される
- [ ] 既存 `pending_discoveries[]` エントリ・`wiki/sources/*.md.source_url` の**再正規化/移行方針**が定義される（phantom duplicate / missed match を防ぐ）
- [ ] 3c の dedup キー（正規化後 url）・3d mode B の同一 source 判定 3 段がフル仕様に追随する

**D 群: stuck candidates 対策（parked・line 410 / 3c で 3e 送り）**
- [ ] 承認しない候補が `pending_discoveries` 首位に居座る failure mode の解消策が定義される（3c では「手動削除」注記で既知の限界として放置していた分）

**E 群: 3c mode G 承認 UX amendment（Tier A・142 件 backfill 対策）**
- [ ] `discover-tier-a` の承認を opt-in 上位 4 件提示から **capped バッチ opt-out**（既定=取り込み・除外を選択）に変更。`AskUserQuestion` は opt-in 仕様のため「**除外したいページを選ぶ（選ばなければ全部取り込み）**」と反転提示する
- [ ] discover 時に各候補ページの**概要（title/説明/冒頭）を軽量先取り**して `pending_discoveries` に併記し、バッチ承認時に表示する
- [ ] **1 run あたり ingest 件数上限 N**（機械的ペース制限・人間ゲートではない）を導入し、未取り込み ~142 件 backfill を複数バッチに分割する
- [ ] backfill（初回大量）と継続差分が**同一経路**で処理される（特別扱いの分岐を作らない）

**F 群: schema / SKILL.md / lint co-evolution**
- [ ] 上記（inbox 構造・概要フィールド・正規化ルール）に伴う schema version bump とボールトポインタ・`log.md` 改訂行
- [ ] SKILL.md に hook 連携・inbox 吸い上げ・amendment 後の mode G UX が追記される
- [ ] CLAUDE.md ステータス行・「主役機能」表が Phase 3e 実装時点で更新される
...
### /llm-wiki Phase 3f（ウォッチリスト Tier B・要件のみ記録／設計は次回 brainstorm・plan）

- [ ] ウォッチリスト schema（`url` / `type`=feed|single / tier B / `label` / `added` 日付）が定義され version bump される
- [ ] **単一 URL 型**: ウォッチリスト登録＝承認 → 取り込み後は自動 refresh（`refresh-tier-a` の Tier B 版）
- [ ] **定点フィード型**: サイト登録＝1回承認（購読モデル）→ 新着 URL を RSS/sitemap で発見（`discover-tier-a` の Tier B 版）
- [ ] **定点フィード自動 ingest に Claude Code relevance フィルタ必須**（判定機構 a=タイトル/URL キーワード／b=cron 内モデル判定／c=2 段、と非関連記事の扱い（drop / negative cache）は 3f 設計で確定）
- [ ] 承認後の refresh/ingest は cron 自動（Tier B バージョン乖離は決定6・矛盾は `## 矛盾` 自動追記で吸収・フィード単位の上限/除去で暴走防止）
- [ ] **X 自動巡回は Phase 4 ソース別 fetcher 依存**として記録（当面は単一 URL 型に留め、自動巡回は RSS/sitemap 取得可能ソースに限定）
- [ ] 取得可否（特に非公式ソースの WebFetch/fetcher）の最小 spike を**着手前 gate** にする（3a 同様）
...
- ウォッチリスト型 Tier B 定点観測（B の残り）（**Phase 3f・要件のみ記録／設計は次回 brainstorm/plan**・2026-05-30 brainstorm で 3e から分離）。`refresh-tier-a`/`discover-tier-a` の **Tier B 版**を新設。**単一 URL 型**（特定記事・X スレッド等）＝ウォッチリスト登録＝承認 → 取り込み後は自動 refresh。**定点フィード型**（X プロフィール・会社ブログ等）＝サイト登録＝1回承認（購読モデル）→ 新着 URL を RSS/sitemap で発見 → **Claude Code relevance フィルタ（判定機構 a=キーワード/b=cron モデル判定/c=2段、は 3f 設計で確定）通過分のみ cron 自動 ingest**。承認後の refresh/ingest は cron 自動（Tier B のバージョン乖離は決定6・矛盾は `## 矛盾` 自動追記で吸収）。**X は公開 RSS 無し・WebFetch 困難＝Phase 4 ソース別 fetcher 依存**のため当面は単一 URL 型に留め、自動フィード巡回は RSS/sitemap 取得可能ソースに限定（sequencing inversion: 3f-feed は Phase 4 と前後して着地）。取得可否 spike を着手前 gate にする（3a 同様）。schema にウォッチリスト構造（url / type=feed|single / tier B / label / added 日付）追加＋ version bump 見込み
...
| Phase 3f 定点フィード（特に X）の取得が技術的に困難（X は公開 RSS 無し・WebFetch 困難） | 中 | X は当面 Tier B 単一 URL 型に留め、自動巡回は RSS/sitemap 取得可能ソースに限定。X 自動巡回は Phase 4 ソース別 fetcher 依存として sequencing inversion を記録・取得可否 spike を着手前 gate に |
| Phase 3f 定点フィード自動 ingest が Claude Code 無関係記事を取り込む（X/会社ブログは他トピックも投稿） | 中 | フィード自動 ingest に Claude Code relevance フィルタを必須化（判定機構は 3f 設計で確定）。false negative は会話 hook/手動 ingest で救済、false positive はフィード単位で除去 |
