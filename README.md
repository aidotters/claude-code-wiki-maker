...
#    ※ cron 運用するなら実体は ~/Documents / ~/Desktop / ~/Downloads の外に置く
#      （これらは macOS TCC 保護フォルダで、launchd ジョブから読み書きできない）
ln -s ~/claude-code-wiki ./wiki-vault
...
- **`ingest`** は取り込み時に要約・相互参照・矛盾照合まで済ませます（あとから検索で繋ぐのではなく、取り込み時に「コンパイル」する）。`--type=practice` / `--feature=<slug>` でページ種別を指定できます。取り込み時に**元ドキュメントの日付**（公開日・最終更新日）もベストエフォートで記録し、`index.md` の各行に `（鮮度: …）` として表示するので、一括取り込みでも新旧が一目で分かります。
- **`query`** は `index.md` → 関連ページの順に読み、引用付きで答えます。Wiki に無い内容は Web 補完を明示します。
- **`synthesize`** の成果物（チートシート等）は再生成・`lint` の対象になります。引用元が古い（鮮度日 180 日超）ときは注記され、`lint` の `source-date-stale`（情報）でも surface されます。
- **`ingest --backfill-dates`**（一回限り）: 既に取り込み済みの raw に、後追いで日付メタを補完します（取れない分は `unknown` のまま・本文は変更しません）。`--dry-run` でプレビュー可。
...
- **launchd で動かすときの 3 つの必須条件**（plist 例に反映済み・詳細はテンプレ冒頭コメント）:
  1. ボールト実体を **`~/Documents` / `~/Desktop` / `~/Downloads` の外**に置く（TCC でアクセス不可になる）。
  2. plist は `claude` を直接起動せず **`/bin/zsh -lc 'exec claude …'`** でラップする（直接 exec は exit 78 で即死）。
  3. **`--permission-mode bypassPermissions`** を付ける（無人実行では Bash が権限分類器で止まるため・信頼済みローカルスキル前提）。
- **夜間スリープする Mac（≠ 常時起動）の場合**: 個別 4 plist の stagger は機能しない（`pmset` の `sleep` が短いと
  ジョブ間で再スリープし後続が発火しない）。`references/llm-wiki-cron-combined.plist.example` の**統合 1 ジョブ**
  （`caffeinate -i` で全モードを連続実行）を使い、**`sudo pmset repeat wake MTWRFSU 06:00:00`** でジョブと同時刻に
  スリープから自動復帰させる。電源オフ運用では自動実行できない（スリープ専用）。
- **このマシンの実構成・残作業チェックリスト・トラブルシュートは [`docs/others/cron-setup-runbook.md`](docs/others/cron-setup-runbook.md) に集約。**
...
**MVP（Phase 1）＋ Phase 2〜5 まで実装・検証済み**（schema v1.11.0）。Phase 4 では Medium 取り込み（per-host content routing ＋ Notion DB ベースの新着発見）、Phase 4c では公式ブログ `claude.com/blog` の定点観測（RSS 不在のため sitemap 巡回・Tier B）、Phase 5 では raw スナップショットへの元日付メタ（`published_at` / `last_modified` ＋ 出所・経路別ベストエフォート抽出）と既存 raw への遡及補完 `--backfill-dates`・鮮度 lint #17・index 代表鮮度日を追加しました。Phase 5 の Phase 2 では Medium 記事の日付抽出を解禁（`scrape-medium --emit-meta` が JSON-LD から `published_at` / `last_modified` を取得し ingest 時に raw へ転記。出所 `html-meta`）。
