...
pending_feed_discoveries:                 # Tier B フィード発見 URL の保留。既定 []
  - url: https://simonwillison.net/2026/Jun/01/claude-code-tips/
    ...
    # last_modified: 2026-06-01            # 任意・Phase 5 v1.11.0。I-3 で抽出した元記事更新日（http-feed の updated / sitemap の lastmod）。I-6 ingest で raw frontmatter にキャリー（出所は feed-pubdate / sitemap-lastmod）。notion-medium-db 由来は持たない（Medium 日付は Phase 2）
...
- **出所 enum**: `gh-commit`（gh api の commit `committer.date`）/ `feed-pubdate`（RSS/Atom の `pubDate`・`updated`・`published`）/ `sitemap-lastmod`（sitemap の `lastmod`）/ `html-meta`（HTML の `article:*_time`・JSON-LD `datePublished`/`dateModified`・構造化メタ）/ `html-body`（**本文可視日付**を保守パースで抽出＝下記・`html-meta` より低信頼）/ `manual`（人手追記）/ `unknown`（取得不能）。
