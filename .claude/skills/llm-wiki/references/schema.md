...
§6 の schema 軽量ポインタは §5 の co-evolution 経路でのみ更新する。両書き込みモードは自分の担当フィールドのみを書き換え、相手モードのフィールドには触れない。`pending_discoveries[].declined`（Phase 3e v1.6.0）はモード G が更新する（G-6 の除外で `true` を立てる・モード F / co-evolution 経路は不可触）。会話 URL（モード H `review`）は vault 外 inbox を queue とし、`pending_discoveries` には書き込まない（Tier A discovery 専用）。
