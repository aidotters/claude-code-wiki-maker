...
### 2.1 `current-baseline.md` 専用フィールド（Phase 3a 追加・v1.3.0 / Phase 3c 拡張・v1.5.0 / Phase 3f 拡張・v1.7.0）

`current-baseline.md` は §2 の共通フィールドと §6 の schema 軽量ポインタに加え、以下の専用フィールドを保持する。**書き込みモード（モード F `refresh-tier-a` / モード G `discover-tier-a` / モード W `refresh-watchlist`）が自動更新する領域**であり、§6 の schema 軽量ポインタとは別領域として共存する（各モードとも §6 を不可触）。
...
| 領域 | モード F `refresh-tier-a` | モード G `discover-tier-a` | モード W `refresh-watchlist`（v1.7.0） |
|------|:------------------------:|:-------------------------:|:-------------------------------------:|
| `last_tier_a_refresh` / `migration_pending` | 更新可 | 不可触 | **不可触** |
| `last_discover_tier_a_run` / `pending_discoveries` | 不可触 | 更新可 | **不可触** |
| `last_refresh_watchlist_run` | 不可触 | 不可触 | **更新可** |
| §2 共通 `claude_code_version` / `updated`（baseline version 系） | 更新可（per-source ingest 由来） | 更新可（共通 surface ingest 由来） | **🚫 不可触（W-4f 省略）** |
| §6 schema 軽量ポインタ（`schema_version` / `schema_repo_commit` / `schema_summary`） | **不可触** | **不可触** | **不可触** |
...
**更新権限の境界（v1.3.0 で明記・v1.5.0 で mode G 追記・v1.7.0 で mode W 追記）**: `schema_version` / `schema_repo_commit` / `schema_summary` の 3 フィールドは **§5 の co-evolution 経路でのみ更新**する。`refresh-tier-a`（モード F）は §2.1 の `last_tier_a_refresh` / `migration_pending`、`discover-tier-a`（モード G）は §2.1 の `last_discover_tier_a_run` / `pending_discoveries`、`refresh-watchlist`（モード W）は §2.1 の `last_refresh_watchlist_run` **のみ**を更新する（モード W は version 系も不可触＝W-4f 省略）。モード F / G は §2 共通フィールドの `claude_code_version` / `updated` も書き換える（詳細は §2.1 編集境界表）、本節の軽量ポインタは全モード**不可触**。
