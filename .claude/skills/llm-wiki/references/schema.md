...
- **`pending_feed_discoveries`**（Phase 3g v1.8.0・Phase 4 v1.9.0 で `summary` 追加）: 配列。既定 `[]`。mode I が stage-1 keyword フィルタ通過の Tier B フィード記事 URL を append（dedup キー = 正規化後 url・既存エントリありなら append skip・`detected_on` は古い方を保持）。対話モードの AskUserQuestion 承認 → 共通 surface ingest 成功で該当エントリを削除。`declined: true` は ①ユーザー選択（`declined_reason` 不在）②stage-2 auto-decline（`declined_reason: relevance`）③cap eviction（`declined_reason: cap`）の 3 種（design §4.2）。フィールド: `{url, feed_source_slug, source_kind: "feed", detected_on, summary?, declined?, declined_reason?}`。`declined_reason` の 3 値: 不在＝ユーザー選択 opt-out / `relevance`＝stage-2 auto-decline（confidence < 0.7）/ `cap`＝cap eviction（per-feed N=50・全体 M=200 を超過した最古エントリを invisible 化）。escape hatch＝`declined` を手動削除で再候補化可。`pending_discoveries[]`（Tier A 専用・mode G 所有）とは**別配列**（queue 分離設計・mode H inbox 分離の前例と整合）。
  - **`feed_source_slug`**: `feed_url` を持つ source ページの slug。**document-less な Notion 由来エントリ（4b）は source ページが無いため固定値 `notion-medium-db`**（= feed_registry の `source` 値）を入れる（Phase 4・mode I I-5）。
...
- **`feed_registry`**（Phase 4 v1.9.0）: 配列。既定 `[]`。**document-less な discovery source（ingest する raw を持たない集約外部ソース）の登録 list**。エントリの形は `{source, selector, label, added}`。`source` = discovery source 種別（現状 `notion-medium-db` の 1 種）、`selector` = ソース内セレクタ（`notion-medium-db` では DB セレクタ・`default` は env 解決＝`NOTION_MEDIUM_DATABASE_ID`→`NOTION_DB_ID_DAILY_DIGEST`・将来複数 DB の予約）、`label` = 人間可読ラベル、`added` = `YYYY-MM-DD` 登録日。**これは発見 URL の保留 array（`pending_*`）ではなく登録 registry** であり、pending array は 3 つ（`migration_pending` / `pending_discoveries` / `pending_feed_discoveries`）のまま。**http(s) RSS/Atom feed は feed_registry に載せず従来どおり source ページの `feed_url` に置く**（feed_registry は document-less ソース専用・§2 `feed_url` 値域不変）。登録は `/llm-wiki ingest --feed=notion-*:<selector>`（path-or-url 省略）の mode B 特例が append（重複 no-op）、retrofit は frontmatter 手動追記。mode I I-3 がこれを読んで巡回対象に足す。**raw-less な source ページを `wiki/sources/` に作らないため lint sources-empty / 不変条件 #2 引用 / mode B raw 前提 pipeline の 3 衝突を回避**（design §4.5）。
...
| `minitools-playwright`（**Phase 4**） | mode B step 3 host routing の Medium 分岐（4a・`scrape-medium --cdp`） | **B** | **英語原文の逐語 Markdown**（要約ではない・`note: English original scraped via minitools Playwright CDP (verbatim, untranslated)`）。Medium raw は §4 tier 判定で **Tier B**（`medium.com` / `*.medium.com`）。verbatim 原則（design D2）で翻訳済みは入れず英語原文を保存する。再検証は `source_url`（原文 URL）を参照 |
