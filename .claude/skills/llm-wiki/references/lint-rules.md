...
- 検査別件数: orphan=A, updated=B, version=C, stale=D, confidence=E, index=F, baseline=G, cross-topic=H, synthesis-stale=I, three-way=J, version-resolve=K, last-tier-a-refresh=L
```
...
| 事象 | 扱い |
|------|------|
| `--check=<csv>` に未知のキーが含まれる | エラーメッセージで中断（許容キー一覧 `orphan|updated|version|stale|confidence|index|baseline|cross-topic|synthesis-stale|three-way|version-resolve|last-tier-a-refresh` を案内） |
