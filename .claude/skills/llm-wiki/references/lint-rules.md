...
これらは MVP/2a 固定（#12 は Phase 3a、#13 は Phase 3c）。`.llm-wiki.json` でのユーザー上書きは Phase 3 以降で検討。
...
| 事象 | 扱い |
|------|------|
| `--check=<csv>` に未知のキーが含まれる | エラーメッセージで中断（許容キー一覧 `orphan|updated|version|stale|confidence|index|baseline|cross-topic|synthesis-stale|three-way|version-resolve|last-tier-a-refresh|last-discover-tier-a-run` を案内） |
...
受け入れ条件の基準は「13 検査 × 2 ケース＝26 ケース」相当。上記カタログは:
...
- 合計 **32 ケース**（26 を上回る粒度）。Phase 3c 完了時に対話レポートで全件が期待通り分類されることを 1 回確認する
