...
これらは MVP/2a 固定（#12 は Phase 3a、#13 は Phase 3c、#14 は Phase 3f）。`.llm-wiki.json` でのユーザー上書きは Phase 3 以降で検討。
...
| 事象 | 扱い |
|------|------|
| `--check=<csv>` に未知のキーが含まれる | エラーメッセージで中断（許容キー一覧 `orphan|updated|version|stale|confidence|index|baseline|cross-topic|synthesis-stale|three-way|version-resolve|last-tier-a-refresh|last-discover-tier-a-run|last-refresh-watchlist-run|watch-fetch-failed` を案内） |
...
受け入れ条件の基準は「15 検査 × 2 ケース＝30 ケース」相当。上記カタログは:
...
- 合計 **37 ケース**（30 を上回る粒度）。Phase 3f 完了時に対話レポートで全件が期待通り分類されることを 1 回確認する
