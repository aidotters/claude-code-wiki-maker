...
3.5. **同一 source 判定（共通 surface・Phase 3d）**
   ...
   **URL 正規化（フル仕様・Phase 3e・本節が単一正本）**: 突合前に URL を次のルールで正規化する。**この正規化規約は mode G G-4 / mode H H-3 からも参照される唯一の正本**であり、それらの節では再記述しない。適用順:
   ...
   4. **tracking param 除去（curated denylist）**: query string のうち **denylist キーのみ**除去する。denylist = `utm_source` / `utm_medium` / `utm_campaign` / `utm_term` / `utm_content` / `fbclid` / `gclid` / `ref` / `ref_src` / `mc_cid` / `mc_eid`。denylist 以外の param（`?page=2` / `?id=123` / `?v=xxx` 等）は**保持**する。
...
Tier B 登録済みフィード（`feed_url` フィールドを持つ source ページ）の RSS/Atom を定期巡回し、新着 URL を発見 → stage-1 keyword フィルタ → `pending_feed_discoveries[]` に dedup append → 対話モードで stage-2 LLM 判定 + capped バッチ opt-out 承認 → 共通 surface（モード B）経由 ingest する（Phase 3g）。`discover-tier-a`（モード G）の Tier B フィード版。
...
### I-3. フィード巡回（feed 登録ソース走査 + RSS/Atom fetch + entry URL 抽出）

1. `Glob('./wiki-vault/wiki/sources/*.md')` でページ列挙、各ページを `Read(offset=0, limit=50)` でフロントマター取得。
2. フロントマター `feed_url` フィールドが存在するページのみ候補（tier は問わない。`feed_url` は source ページ自身のフロントマターに格納される独自フィールドで、**raw を辿る必要はない**）。
3. `feed_url` が 0 件の場合: log `discover-watchlist: no registered feeds (YYYY-MM-DD)` を追記し、I-7 heartbeat のみ更新して正常終了。
4. 各候補の `feed_url` を `Bash("curl -sL --max-time 30 --max-filesize 2097152 <feed_url>")` で逐語取得。**WebFetch 不使用**（G-3 sitemap 同理由＝WebFetch は要約するため entry URL を取りこぼす）。
   - 取得失敗（timeout / 4xx / 5xx）は当該フィードを skip + log `discover-watchlist: feed fetch failed <slug>: <error>`。他フィード続行。
5. 取得成功した XML から entry URL を抽出（bash テキスト処理):
...
6. 結果は `(source_slug, [entry_url, ...])` のリスト（`source_slug` ASCII 昇順）。
...
- **stage-1 keyword フィルタ（hardcoded・LLM API コスト 0）**: 各 entry の title と URL を対象に以下の keyword セットで `grep -iE` でフィルタ:
  ...
  ```
  タイトル抽出は RSS `<title>` タグ（CDATA 含む）または Atom `<title>` タグを使う。タイトル **OR** URL のいずれかにヒットでパス。ヒットしない entry は `pending_feed_discoveries` に append しない（粗ふるい）。stage-1 は cron（`--no-prompt`）実行中に走り、LLM API コストが発生しない。
...
- エントリ形式: `{url: <正規化後 URL>, feed_source_slug: <slug>, source_kind: feed, detected_on: <今日>}`（`references/schema.md` §2.1）。
...
1. **候補抽出**: `pending_feed_discoveries[]` から **`declined: true` を除いた**エントリを `detected_on` ASC → URL ASCII 昇順 fallback で並べ、先頭 **min(N, 残数)** 件を本 run の処理対象（N = per-run cap・既定 20）。
2. **ラウンド処理**（4 件/ラウンド・最大 ⌈N/4⌉ ラウンド）。各ラウンド:
   1. **stage-2 統合の遅延取得**: 当該バッチの**最大 4 件のみ** `WebFetch(url, "この記事が Claude Code / Claude API / LLM エージェント開発に関連するかを次の JSON で答えよ: {\"relevant\": true|false, \"confidence\": 0.0-1.0, \"summary\": \"1文要旨\"}")` を実行。
      - `relevant: false` または `confidence < 0.7`: 当該エントリに `declined: true, declined_reason: relevance` を立て（stage-2 auto-decline）バッチから除去。AskUserQuestion に出さない。
      - WebFetch 失敗: 概要なし・relevance 判定スキップで続行（ユーザーに提示）。
   ...
   5. **選択されなかった（＝取り込む）候補**: 各 URL について **モード B の ingest 本体（step 3 raw 確保 〜 step 9 commit、step 0 lock / step 10 解放は除外）**を内部呼び出し。
      - `--watch` は渡さない（`--watch` 非伝播・`--feed` 非伝播）。
...
3. **declined 操作ログ**: stage-2 auto-decline（`declined_reason: relevance`）とユーザー除外（`declined_reason` 不在）を区別して I-8 サマリに反映（G-8 の opted-out 可視化と同流儀）。
4. overview 自動更新はモード B step 8.5 が担当（mode I 単独の overview 更新 step は持たない）。
...
フィード巡回 + stage-1 フィルタ + `pending_feed_discoveries` append（cap/eviction 含む）のみ実行。AskUserQuestion 不発火・stage-2 LLM 不実行（LLM API コスト 0）。cron 運用で `launchd` / `cron` から `claude --print '/llm-wiki discover-watchlist --no-prompt' --allowedTools=Read,Write,Edit,Bash,Grep,Glob,WebFetch` 形式で起動。stagger 時刻: refresh-tier-a（03:00）/ refresh-watchlist（03:30）/ discover-watchlist（**04:00**・既定例）。起動時刻 plist は `references/discover-watchlist-launchd.plist.example` を参照。
...
| `feed_url` を持つ source ページが 0 件 | I-3 で対象空・log `discover-watchlist: no registered feeds (YYYY-MM-DD)` + I-7 heartbeat のみ更新して正常終了 |
