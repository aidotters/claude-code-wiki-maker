...
- **値が本日付と異なる場合**: フロントマターを本日付に Edit し、1 commit `refresh(tier-a): last_tier_a_refresh = <date>`。
- `--dry-run` モードではいずれの分岐も commit を行わず、`would-update last_tier_a_refresh <old> -> <today>` または `last_tier_a_refresh unchanged (<date>)` を標準出力にレポートするのみ。
