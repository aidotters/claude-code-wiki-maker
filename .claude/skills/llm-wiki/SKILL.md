...
argument-hint: "<init|ingest <path-or-url> [--type=practice|--feature=<slug>]|query <質問>|synthesize <テーマ>|lint [--check=<csv>]|refresh-tier-a [--dry-run]|discover-tier-a [--no-prompt|--dry-run]|review [--dry-run]>"
---
...
`.llm-wiki.lock` は **書き込みモード**（B / D / F / G / H、および lint #11 承認制決着の `## 矛盾` 編集部分）でのみ取得する。`query`（モード C）は読み取り専用のため不要。`lint` 通常実行（11/12/13 検査レポート＋ `wiki/log.md` 追記まで）も**不要**（design.md §6 carve-out）。

- **ファイル**: vault 直下 `.llm-wiki.lock`（`/llm-wiki init` で vault `.gitignore` に追加済）。
- **フォーマット**: 1 行 JSON `{"pid": <PID>, "started_at": "<ISO8601>", "mode": "ingest|synthesize|refresh-tier-a|discover-tier-a|review|lint-resolve"}`
