...
argument-hint: "<init|ingest <path-or-url> [--type=practice|--feature=<slug>]|query <質問>|synthesize <テーマ>|lint [--check=<csv>]|refresh-tier-a [--dry-run]>"
---
...
/llm-wiki lint [--check=<csv>]                          # 12 検査（Phase 2a 7 + Phase 2b 4 + Phase 3a 1、#11 のみ承認制で書き込み）
...
2. 分岐:
   ...
   - `lint` → **モード L**（Phase 2a 7 + Phase 2b 4 + Phase 3a 1 の 12 検査）
...
`.llm-wiki.lock` は **書き込みモード**（B / D / F、および lint #11 承認制決着の `## 矛盾` 編集部分）でのみ取得する。`query`（モード C）は読み取り専用のため不要。`lint` 通常実行（11/12 検査レポート＋ `wiki/log.md` 追記まで）も**不要**（design.md §6 carve-out）。

- **ファイル**: vault 直下 `.llm-wiki.lock`（`/llm-wiki init` で vault `.gitignore` に追加済）。
- **フォーマット**: 1 行 JSON `{"pid": <PID>, "started_at": "<ISO8601>", "mode": "ingest|synthesize|refresh-tier-a|lint-resolve"}`
...
0.b **migration_pending 提案**（書き込み前・lock 取得直後）: モード L §ステップ 2.5 の手順を**同一ロジックで適用**（候補 1 件でも「適用しない」併置、`multiSelect: true`、上位 4 件、承認分は source ページ本文 URL 書き換え＋当該エントリ削除＋ 1 commit）。0 件選択は通常 ingest 続行。
...
**Phase 3a 機械判定 1 検査**（#12 last-tier-a-refresh、書き込みなし）を実行する。
...
1. **引数パース**
   - 第 2 トークン以降から `--check=<csv>` を抽出。`<csv>` は
     `orphan|updated|version|stale|confidence|index|baseline|cross-topic|synthesis-stale|three-way|version-resolve|last-tier-a-refresh`
     のサブセット（カンマ区切り）。
   - 未指定なら全 12 検査。未知キーはエラーで中断し、上記 12 キー一覧を案内。

2. **ステップ 0.5 ボールト前提チェック**を継承（init 以外と同じ手順）。**ステップ 0.6 lock は通常 lint では取得しない**（design.md §6 carve-out）。

2.5. **migration_pending 提案**（書き込み前・通常 lint 検査群の手前）:
...
5. **対話出力**
   - Markdown 表（カラム: file / check / severity / detail）で全 12 検査の結果を統合出力。
...
`Bash('git -C ./wiki-vault status --porcelain')` の出力が**非空**なら、`wiki/log.md` に
`refresh-tier-a: vault dirty, skipped (YYYY-MM-DD)` を追記、lock を解放して終了（stash / 自動 commit 禁止）。
...
- フロントマター: `source_url` / `fetched_at` / `fetched_via: WebFetch` / `tier: A` / `note: WebFetch 要約（逐語性ポリシーは memory/webfetch-raw-snapshot-policy.md 参照）` を必ず記録。
...
**F-4g. per-source commit**:
- ボールト側 Git で当該ソース関連の差分のみを `git add` し、`refresh(tier-a): <slug> at <取得日>` でコミット。
- **`git push しない`**。

### F-5. 全ソース処理後の last_tier_a_refresh 更新

- `current-baseline.md` フロントマターの `last_tier_a_refresh` を**本日付**に更新。
- 1 commit `refresh(tier-a): last_tier_a_refresh = <date>`。
- `--dry-run` モードではこの commit も行わない。
...
| 301 リダイレクト | F-4b で `migration_pending` append（既出はサプレッション）、当該ソース再コンパイルなし |
