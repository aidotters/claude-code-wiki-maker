...
argument-hint: "機能名, --from-idea [ファイル名], --from-plan [ファイル名], --new 機能名"
---
...
**注:** `product-requirements.md` は中規模以上の新規プロジェクトでのみ生成されます。
存在しない場合（小規模プロジェクト等）はこのステップをスキップし、機能名を直接 requirements.md の起点として使用してください。

1. **PRDの読み込み**
   ```
   Read('docs/core/product-requirements.md')  # 存在する場合のみ。未生成ならスキップしてステップ2へ
   ```
...
2. **docs/core/のドキュメントを目的別に確認する**
   ...
   **注:** プロジェクト規模（小規模〜大規模）に応じて、生成されているドキュメントは異なります。
   `gen-all-docs` の構成に従い、**存在するファイルのみ読み込み**ます。未生成のドキュメントについてはエラーとせず次へ進みます。

   **真のコア（存在する想定。読めない場合は警告のみ）:**
   ```
   ...
   **スケーラブルオプション（中規模以上で生成。存在時のみ確認）:**
   ```
   Read('docs/core/product-requirements.md')  # 新規プロジェクト時
   ```
   ...
   **条件付き（大規模で生成。必要に応じて確認）:**
   ```
   Read('docs/core/api-reference.md')  # 既存APIを拡張する場合
   Read('docs/core/diagrams.md')       # データフローを変更する場合
   Read('docs/core/functional-design.md')  # 機能仕様の参照が必要な場合
   Read('docs/core/glossary.md')           # ドメイン用語の確認が必要な場合
   ```

   **小規模プロジェクトの補完:**
   `architecture.md` や `repository-structure.md` が未生成の場合は、`README.md` と `CLAUDE.md`、および実コード（`src/`）から構造を直接把握します。
