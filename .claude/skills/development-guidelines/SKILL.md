...
1. `docs/core/architecture.md` (アーキテクチャ設計書) - 技術スタックの確認
2. `docs/core/repository-structure.md` (リポジトリ構造) - ディレクトリ構造の確認

`gen-all-docs` の規模別生成方針により、上記は **中規模以上**でのみ生成されます。
未生成の場合（小規模プロジェクト）は、以下のフォールバックを使用してください:

| 状況 | フォールバック |
|------|---------------|
| 小規模プロジェクト（両方未生成） | `README.md` + `CLAUDE.md` + 実コード（`src/`、`pyproject.toml` 等）から技術スタックとディレクトリ構造を把握 |
| 既存プロジェクトモード | 実コードのコーディングパターンを優先的に参照 |

開発ガイドラインは、把握した技術スタックとディレクトリ構造に
基づいた具体的なコーディング規約と開発プロセスを定義します。

**注:** 小規模プロジェクトでは、生成する `development-guidelines.md` も最小構成（テスト・リント・主要規約のみ）でよい。

## 既存ドキュメントの優先順位

**重要**: `docs/core/development-guidelines.md` に既存の開発ガイドラインがある場合、
以下の優先順位に従ってください:

1. **既存の開発ガイドライン (`docs/core/development-guidelines.md`)** - 最優先
