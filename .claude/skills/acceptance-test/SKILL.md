...
argument-hint: "[.steeringパス], --from-idea [ファイル名]"
---
...
```
# 例: 「.env に XXX 項目がある」または「設定モジュールに設定がある」
Grep('XXX=', '.env')
# 言語別の設定モジュール（例: src/config.py / src/config.ts / internal/config/*.go / src/config.rs）
# MCP-only モードでは `.mcp.json` の `env` セクションを参照
→ 見つかれば PASS、なければ FAIL
```
...
```bash
# 検出言語のテストランナーで該当テストを実行
#   Python: pytest tests/test_xxx.py -v
#   TS:     vitest run tests/xxx.test.ts
#   Rust:   cargo test xxx
#   Go:     go test ./... -run TestXxx
Bash('{検出言語のテストコマンド} {対象テスト}')
→ テストパスすれば PASS、失敗すれば FAIL

# MCP-only モードではテストを実行せず、`.mcp.json` の構文検証 + 該当 MCP サーバーの起動コマンド検証で代替
```

### 検証ルール

言語非依存に「公開シンボル」「設定項目」「副作用」を検証する。検出言語に応じて Grep のパターンを切り替える:

| 条件パターン | 言語別検証方法 |
|--------------|----------------|
| 「〜クラス/型が存在する」 | Python: `class {名前}` / TS: `class {名前}` or `interface {名前}` / Go: `type {名前} struct` / Rust: `struct {名前}` |
| 「〜関数が実装されている」 | Python: `def {名前}` / TS: `function {名前}` or `const {名前} =` / Go: `func {名前}` / Rust: `fn {名前}` |
| 「〜設定項目がある」 | `.env` / 言語別設定モジュール / `settings.yaml` / `.mcp.json` を Grep |
| 「〜ファイルが作成される」 | `Glob('{ファイルパターン}')` |
| 「〜エラーが処理される」 | Python: `except\|try:` / TS: `try \{\|catch` / Go: `if err != nil` / Rust: `Result\|\?` |
| 「〜ログが出力される」 | 各言語のログAPIパターン（例: Python `logger\.\|logging\.`、TS `console\.\|logger\.`、Go `log\.\|slog\.`） |
| 「〜MCP サーバーが利用可能」（MCP-only） | `.mcp.json` の `mcpServers.{名前}` に該当エントリが存在 |
...
- [ ] **条件**: Slack通知が正しく送信される
  - **確認手順**:
    ...
    2. `python -m minitools-xxx --slack` を実行
    3. Slackチャンネルで通知を確認
...
- [ ] **条件**: 100件の記事を3分以内に処理できる
  - **確認手順**:
    1. `time python -m minitools-xxx --max-results 100` を実行
    2. 実行時間を確認
  - **期待結果**: 3分以内に完了
```
