...
# ケース3: ファイル名指定時（.mdを含むか既知のドキュメント名）
              ...
              'diagrams', 'CHANGELOG', 'CLAUDE', 'README',
              'product-requirements'] or 引数.endswith('.md'):
    # ドキュメントファイルを特定
    if 引数 == 'CLAUDE' or 引数 == 'CLAUDE.md':
        Read('CLAUDE.md')
    elif 引数 == 'README' or 引数 == 'README.md':
        Read('README.md')
    elif 引数 == 'CHANGELOG' or 引数 == 'CHANGELOG.md':
        Read('CHANGELOG.md')  # ルート配置
    else:
...
# ケース4: モジュール名指定時（上記以外 → モジュール名と判定）
    ...
    モジュール名 = 引数
    # モジュールの存在確認（言語非依存の拡張子で検出）
    Glob('core/{モジュール名}/**/*.{py,ts,tsx,js,jsx,go,rs}')
    Glob('src/{モジュール名}/**/*.{py,ts,tsx,js,jsx,go,rs}')
...
**真のコアドキュメント（規模を問わず存在前提・必須レビュー対象）:**

| ファイル | 配置 | 内容 | 備考 |
|----------|------|------|------|
| `README.md` | リポジトリルート | プロジェクト概要・使い方 | **常にレビュー対象** |
| `CLAUDE.md` | リポジトリルート | 開発ガイダンス・AI支援用情報 | **常にレビュー対象** |
| `development-guidelines.md` | `docs/core/` | 開発ガイドライン | `dev-guidelines.md`の場合あり |

**スケーラブルオプション (docs/core/, 存在時のみレビュー対象):**

| ファイル | 内容 | 生成される規模 | 備考 |
|----------|------|---------------|------|
| `architecture.md` | アーキテクチャ設計 | 中規模以上 | |
| `repository-structure.md` | リポジトリ構造 | 中規模以上 | `repo-structure.md`の場合あり |
| `product-requirements.md` | プロダクト要件定義 | 中規模以上（新規プロジェクト） | |
| `functional-design.md` | 機能設計 | 大規模 | |
| `glossary.md` | 用語集 | 大規模 | |
| `api-reference.md` | API仕様 | 大規模（API提供時） | |
| `diagrams.md` | Mermaid図 | 大規模 | |

**オプションドキュメント（リポジトリルート）:**

| ファイル | 内容 | 備考 |
|----------|------|------|
| `CHANGELOG.md` | 変更履歴 | 存在時のみ |

**注:** 小規模プロジェクト（Claude Code ツール等）では「真のコア」3点のみが存在する想定。レビュー対象は実在するファイルのみとし、未生成のドキュメントは「未生成」として警告を出さない。
...
if steeringフォルダ指定あり:
    ...
    Glob('core/**/*.{py,ts,tsx,js,jsx,go,rs}')
    Glob('src/**/*.{py,ts,tsx,js,jsx,go,rs}')
→ モジュールリストを作成

# MCP-only モードでは上記の代わりに `.mcp.json` から MCP サーバー名のリストを抽出
...
```
# 公開クラス/関数を抽出（言語別の公開シンボルパターン）
#   Python: 'class [A-Z]|^def [a-z]'
#   TS/JS:  '^export (function|class|const|interface|type)'
#   Go:     '^func [A-Z]|^type [A-Z]'
#   Rust:   '^pub (fn|struct|enum|trait)'
PUBLIC_SYMBOL_PATTERN = {検出言語の公開シンボル正規表現}

if steeringフォルダ指定あり:
    # design.mdから特定した実装ファイルのみ対象
    for ファイルパス in 実装ファイルリスト:
        Grep(PUBLIC_SYMBOL_PATTERN, ファイルパス)
elif モジュール指定あり:
    Grep(PUBLIC_SYMBOL_PATTERN, 'core/{モジュール名}/')
else:
    Grep(PUBLIC_SYMBOL_PATTERN, 'core/')
→ 公開API一覧

# MCP-only モードでは公開 API の代わりに `.mcp.json` の `mcpServers` 各エントリを「公開ツール」として扱う
...
| 設定項目カバレッジ | 100% | `.env` + 言語別設定モジュール（`src/config.py` / `src/config.ts` / `internal/config/*.go` / `src/config.rs` 等）または `.mcp.json` + 文書比較 |
...
| 項目 | 確認内容 |
|------|----------|
| クラス/型/構造体名 | 実際のシンボル名と一致 |
| 関数・メソッドシグネチャ | 引数・戻り値が一致 |
| ファイルパス | 実際に存在 |
| 設定キー | `.env` または言語別設定モジュール（`src/config.py` / `src/config.ts` / `internal/config/*.go` / `src/config.rs` / `.mcp.json` 等）に存在 |
| コマンド | パッケージ定義ファイル（`pyproject.toml` `[project.scripts]` / `package.json` `bin`・`scripts` / `Cargo.toml` `[[bin]]` / `cmd/*/main.go`）または `.mcp.json` に定義 |

### 4.4 README.md のチェック

| 項目 | 確認内容 |
|------|----------|
| プロジェクト構造 | 検出ソースディレクトリ（`src/` / `lib/` / `cmd/` 等）と一致 |
| コマンド名 | パッケージ定義のスクリプト・bin、または `.mcp.json` の `mcpServers` と一致 |
...
| 項目 | 確認内容 |
|------|----------|
| コンポーネント一覧 | 検出ソースディレクトリのモジュール構造と一致 |
| コマンド名 | パッケージ定義のスクリプト・bin、または `.mcp.json` の `mcpServers` と一致 |
| 外部依存関係 | パッケージ定義ファイル（`pyproject.toml` / `package.json` / `Cargo.toml` / `go.mod`）または `.mcp.json` の依存関係と一致 |
