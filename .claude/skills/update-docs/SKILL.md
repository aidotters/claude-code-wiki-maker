...
**前提:** `/initial-setup` で検出した言語/モードに応じて拡張子・参照ファイルを切り替える。
**MCP-only モードでは本ステップはスキップし、代わりに `.mcp.json` を Read して MCP サーバー一覧を抽出する。**

1. **コアモジュールのスキャン**
   ```
   # 言語別の拡張子で検出
   Glob('src/**/*.{py,ts,tsx,js,jsx,go,rs}')
   ```
   - 各モジュールのクラス、関数、構造体、データクラスを抽出
   - 公開API（公開シンボル: Python はアンダースコアなし、TS は `export`、Go は大文字始まり、Rust は `pub` 等）をリストアップ

2. **エントリーポイント / CLI 定義の確認**

   | 言語 | 参照先 |
   |------|--------|
   | Python | `pyproject.toml` の `[project.scripts]` |
   | TypeScript・Node.js | `package.json` の `bin` / `scripts` |
   | Rust | `Cargo.toml` の `[[bin]]` |
   | Go | `cmd/*/main.go` |
   | MCP-only | `.mcp.json` の `mcpServers` 各エントリ |

   ```
   Read('{検出言語のエントリーポイント定義ファイル}')
   ```
   - 定義されているコマンド/エントリポイント/MCP サーバーをリストアップ

3. **設定ファイルの確認**
   ```
   Read('.env.example')   # または `.env.sample`
   # 言語別の設定モジュール
   #   Python: src/config.py / src/settings.py
   #   TS:     src/config.ts / src/env.ts
...
2. **生成ドキュメント**
   ...
   ルートに配置されるドキュメント:
   ```
   Read('CHANGELOG.md')           # オプション・大規模時のみ
   ```

   **注:**
   - ファイル名は省略形（`repo-structure.md`, `dev-guidelines.md`）で存在する場合もある
   - `gen-all-docs` ではプロジェクト規模に応じて生成範囲が異なる。存在しないドキュメントは更新対象から除外し、警告は出さない
...
| カテゴリ | コードから抽出（言語非依存） | ドキュメントから抽出 |
|----------|------------------------------|---------------------|
| Models | モデル/型定義モジュール（例: `src/models/`） | architecture.md, api-reference.md（存在時） |
| Utils | ユーティリティモジュール（例: `src/utils/`） | architecture.md, api-reference.md（存在時） |
| 前/後処理 | 前処理・後処理モジュール（例: `src/preprocess/`, `src/postprocess/`） | architecture.md, api-reference.md（存在時） |
| Config | 設定モジュール（言語別: `src/config.py` / `src/config.ts` / `internal/config/*.go` / `src/config.rs` 等） | architecture.md, api-reference.md（存在時） |
| CLI / エントリ | パッケージ定義（`pyproject.toml` / `package.json` / `Cargo.toml` / `go.mod`）+ エントリファイル | repository-structure.md（存在時）, README.md |
| 設定項目 | `.env`、設定モジュール | api-reference.md（存在時）, README.md |
| MCP（MCP-only / 併用時） | `.mcp.json`、`.claude/.mcp.json` | README.md, CLAUDE.md, `mcp-configuration.md`（存在時） |

**注:** 小規模プロジェクトでは `architecture.md`、`repository-structure.md`、`api-reference.md` が未生成の場合があります。その場合は `README.md` と `CLAUDE.md` のみが更新対象となります。
...
- 検出言語のエントリポイント（CLI ランナー / `bin` / `[[bin]]` / `cmd/*/main.go` / `.mcp.json` の `mcpServers`）からコマンド一覧を更新
- 新しいコーディングパターンがあれば追加
- 「次の実装予定」を現状に合わせて更新
...
### 4.x 共通方針: 存在しないドキュメントの扱い

`gen-all-docs` ではプロジェクト規模に応じて生成されるドキュメントが異なります（小規模では `README.md` + `CLAUDE.md` + `development-guidelines.md` のみ）。
このため、4.1〜4.9 のドキュメントは**存在する場合のみ更新**します。存在しないドキュメントについては、

- スキップする（エラーとしない）
- 必要であれば `/gen-all-docs` で生成を促す案内を最終サマリーに含める
...
- [ ] 「出力形式」が実際の出力と一致している
- [ ] 「設定クラス構造」が言語別の設定モジュール（例: `src/config.py` / `src/config.ts` / `internal/config/*.go` / `src/config.rs`、または `.mcp.json`）と一致している
