...
プロダクトのアイデアについて、ユーザー自身がClaude Codeとの対話を通じてブラッシュアップを完了している必要があります（`/brainstorm` の利用を推奨）。

### docs/ideas/ にアイデアファイルが存在する

**重要**: 壁打ちの内容は、ユーザーが `docs/ideas/` 配下に Markdown ファイルとして保存する必要があります。

**ファイルパス**: `docs/ideas/*.md`（任意のファイル名）

ファイル名の例:
- `docs/ideas/initial-requirements.md`
- `docs/ideas/20250115-auth-feature.md`
- `docs/ideas/20250120-api-design.md`
...
`docs/ideas/` に複数のファイルが存在する場合は、以下のいずれかでターゲットを特定してください:

1. ユーザーから明示的にファイル名が指定されている場合 → そのファイルを使用
2. 1つしか存在しない場合 → そのファイルを使用
3. 複数存在し指定がない場合 → ユーザーに選択を促すか、最新ファイルを使用

PRD作成時には、選択したファイルの内容を参照して詳細化を行います。

## 既存ドキュメントの優先順位

**重要**: `docs/core/product-requirements.md` に既存のPRDがある場合、
以下の優先順位に従ってください:

1. **既存のPRD (`docs/core/product-requirements.md`)** - 最優先
   - プロジェクト固有の要件が記載されている
   - このスキルのガイドより優先する
...
# 選択したファイルを読み込み
Read('docs/ideas/{選択したファイル名}.md')
```

選択ロジックは「アイデアファイルの選択」セクションを参照してください。

### 2. PRDドラフトの生成

選択したアイデアファイルの内容をもとに、テンプレートに従ってPRDを生成します。
...
1. **`docs/ideas/*.md` をベースに作成**: ユーザーが作成した壁打ち内容を参照
