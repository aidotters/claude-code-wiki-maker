---
name: steering
description: 作業計画、タスクリストを記録するためのスキル。ユーザーからの指示をトリガーとした作業計画時、実装時、検証時に読み込む。
...
2. **永続ドキュメントの確認**
   ...
   `gen-all-docs` が生成する `docs/core/` 配下のドキュメントを参照してプロジェクト方針を理解する。
   プロジェクト規模に応じて生成範囲が異なるため、**存在するファイルのみ**読み込み、未生成のものはエラーとせず次へ進む。

   **真のコア（規模を問わず存在する想定）:**
   - `docs/core/development-guidelines.md`

   **スケーラブルオプション（中規模以上で生成。存在時のみ参照）:**
   - `docs/core/product-requirements.md`（新規プロジェクト時のみ）
   ...
   **条件付き（大規模で生成。存在時のみ参照）:**
   ...
   小規模プロジェクトでスケーラブルオプションが未生成の場合は、`README.md` と `CLAUDE.md`、および実コード（`src/`）から構造を把握する。
...
**3-4. 実装を実行**
```
開発ガイドライン（docs/core/development-guidelines.md）に従って実装
```
