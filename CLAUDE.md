...
> **ステータス: MVP（Phase 1）＋ Phase 2a・2b・3a・3b・3d・3c・3e・3f・3g・4 実装済み**
...
> Phase 3g `#16 last-discover-watchlist-run` 機械判定 1 検査追加＝16 検査）/ `refresh-tier-a [--dry-run]`（Phase 3a・Tier A 日次自動再取得・
...
> 早期 return guard を「feed_url 0 件 AND feed_registry 空」両方真に改修・I-6 stage-2 を source 種別で条件分岐〔notion は永続 `summary` 入力で WebFetch 省く・http-feed は従来 WebFetch〕＋ Notion 由来 ingest cap K=5〔Playwright sequential・対話のみ・http-feed ingest は非 cap で非破壊〕・
> `references/discover-watchlist-launchd.plist.example` 同梱〔3 系統 stagger 04:00・Phase 4 で env に `NOTION_API_KEY` 追記・4 系統目 plist は作らない〕）と
...
> `pending_feed_discoveries[].summary`（§2.1・notion-medium-db 由来のみ）を追加＋ `fetched_via: minitools-playwright`（§3・Tier B verbatim）＋ URL 正規化 denylist に `source`〔`feed_url` 値域は不変〕を追加）を `.claude/skills/llm-wiki/` に実装済みです。
...
| `/llm-wiki ingest <path-or-url> [--type=practice|--feature=<slug>] [--watch] [--feed=<rss_url>|--feed=notion-*:<sel>]` | ソース取り込み・コンパイル・相互参照（`--type`／`--feature` は 2a／3d: 共通 surface 拡張で migration_pending 承認後 ingest を内包・同一 source 判定 3 段・sources: 末尾 append・overview 自動更新 inline・`--watch` は 3f: Tier B watchlist 登録・`--feed=<rss_url>` は 3g: Tier B フィード登録〔`feed_url` を立て mode I 巡回対象に〕・**4a**: step 3 host routing で `medium.com` → `scrape-medium --cdp` 英語原文 raw〔対話のみ・Medium+`--watch` は reject〕・**4b**: `--feed=notion-*:<sel>` は document-less ソースを `feed_registry[]` に registry append〔ingest short-circuit・raw/source ページなし〕） | 1（MVP）＋ 2a ＋ 3d ＋ 3f ＋ 3g ＋ 4 |
...
| `/llm-wiki lint [--check=<csv>]` | 健全性・陳腐化・信頼度の監査（2a: 機械判定 7 検査・レポートのみ／2b: 意味解釈 4 検査・承認制／3a: #12 last-tier-a-refresh 機械判定／3d: §2.5 migration_pending 提案フローを共通 surface 経由に再定義／3g: #16 last-discover-watchlist-run 機械判定追加＝16 検査） | 2a／2b／3a／3d／3g |
...
- ボールト整合の検証: `/llm-wiki lint`（Phase 2a 機械判定 7 検査＋ Phase 2b 意味解釈 4 検査＋ Phase 3a/3c/3f/3g 機械判定 5 検査＝16 検査・実装済み。#11 のみ承認制で `## 矛盾` 末尾に決着注記を追記）
