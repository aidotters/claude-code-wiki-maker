...
> 主役機能 `/llm-wiki` の `init` / `ingest` / `query` / `synthesize` と schema/templates を
> `.claude/skills/llm-wiki/` に実装済みです（`lint` は Phase 2）。スキル改修後は `/update-docs` で本ファイルを実態へ同期してください。
...
- ボールト整合の検証: `/llm-wiki lint`（Phase 2・未実装。MVP では未提供）
