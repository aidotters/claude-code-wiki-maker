...
> `pending_feed_discoveries[].summary`（§2.1・notion-medium-db 由来のみ）を追加＋ `fetched_via: minitools-playwright`（§3・Tier B verbatim）＋ URL 正規化 denylist に `source`〔`feed_url` 値域は不変〕を追加・v1.10.0 で `feed_registry[]` の既知 source に `claude-blog-sitemap`〔§2.1・Phase 4c・RSS 不在の公式ブログを sitemap 巡回〕と in-memory `route_kind` 語彙に `sitemap` を追加〔§4 tier 判定表は不可触＝blog は既存「任意ブログ = Tier B」で充足〕）を `.claude/skills/llm-wiki/` に実装済みです。
