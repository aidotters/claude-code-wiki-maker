...
> **ステータス: MVP（Phase 1）＋ Phase 2a・2b・3a・3b・3d・3c・3e・3f 実装済み**
...
> 共通 surface 経由に再定義＋ Phase 3f `#14 last-refresh-watchlist-run` / `#15 watch-fetch-failed` 機械判定 2 検査追加＝15 検査）/ `refresh-tier-a [--dry-run]`（Phase 3a・Tier A 日次自動再取得・
...
> launchd/cron は refresh-tier-a と起動時刻を stagger＝lock 競合回避・`references/refresh-watchlist-launchd.plist.example` 同梱）と
...
> `last_refresh_watchlist_run`（§2.1・mode W run heartbeat）を追加）を `.claude/skills/llm-wiki/` に実装済みです。
...
6. **情報源ティア**: Tier A（Anthropic 公式ドキュメント/公式 GitHub）は Phase 3a で日次自動更新（モード F `refresh-tier-a`）を先行解禁・実装済み。`current-baseline.md` は Tier A 由来は自動更新可・手動上書き可、Tier B はバージョン乖離時に対話で更新提案（承認制）。書き込みモード（B / D / F / W／lint #11 承認制決着）は `.llm-wiki.lock`（vault 直下・atomic 取得・スタール判定 timestamp 1h ＋`kill -0` の AND）で排他制御する。Tier B watchlist は Phase 3f で日次自動再取得（モード W `refresh-watchlist`・`tier:B`＋`watch:true`）を解禁したが、**baseline の version 系は自動更新しない**（W-4f 省略・決定6＝Tier B は承認制）。
