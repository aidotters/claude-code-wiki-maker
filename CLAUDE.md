...
> **ステータス: MVP（Phase 1）＋ Phase 2a・2b・3a・3b・3d・3c・3e・3f・3g 実装済み**
...
> `references/discover-watchlist-launchd.plist.example` 同梱〔3 系統 stagger 04:00〕）と
...
> `last_discover_watchlist_run` / `pending_feed_discoveries[]`（§2.1・mode I 専用）を追加）を `.claude/skills/llm-wiki/` に実装済みです。
...
| `/llm-wiki ingest <path-or-url> [--type=practice|--feature=<slug>] [--watch] [--feed=<rss_url>]` | ソース取り込み・コンパイル・相互参照（`--type`／`--feature` は 2a／3d: 共通 surface 拡張で migration_pending 承認後 ingest を内包・同一 source 判定 3 段・sources: 末尾 append・overview 自動更新 inline・`--watch` は 3f: Tier B watchlist 登録・`--feed` は 3g: Tier B フィード登録〔`feed_url` を立て mode I 巡回対象に〕） | 1（MVP）＋ 2a ＋ 3d ＋ 3f ＋ 3g |
